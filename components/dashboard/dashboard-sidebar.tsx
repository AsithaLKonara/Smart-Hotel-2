...
export default function DashboardSidebar() {
 ...
 <p className="text-xs text-primary font-bold">Experience Elite</p>
 </div>
 )}
 </Link>
 </div>

 <div className={cn("p-4 border-b border-border transition-all duration-300", isCollapsed && "p-2")}>
 <div className={cn("flex items-center space-x-3 px-3 py-2 rounded-xl bg-muted/50", isCollapsed && "px-1 justify-center")}>
 ...
 <p className="text-xs text-muted-foreground truncate ">{rawRole || 'Guest'}</p>
 </div>
 )}
 </div>
 ...
 isActive ? 'bg-primary/20 text-primary border border-primary/30 shadow-sm' : 'text-white/50 hover:bg-muted/50 hover:text-white',
 ...
 className="w-full flex items-center justify-between px-3 py-2 text-xs font-bold text-muted-foreground tracking-wider hover:text-white/70 transition-colors"
 >
 <span>{groupName}</span>
 <ChevronDown className={cn("w-4 h-4 transition-transform", isOpen ? "rotate-180" : "rotate-0")} />
 </button>
 {isOpen && (
 <div className="space-y-1 pl-2 border-l border-border ml-4">
 {items.map((item) => {
 const Icon = item.icon
 const isActive = pathname === item.href
 return (
 ...
 isActive ? 'text-primary bg-primary/10' : 'text-white/50 hover:text-white hover:bg-muted/50'
 )}
 >
 ...
 {isCollapsed && Object.keys(groupedItems).map((groupName) => (
 <div key={groupName} className="space-y-1 pt-2 border-t border-border">
 ...
 variant="outline"
 className={cn(
 "w-full transition-all duration-300 border-border bg-muted/50 text-white/60 hover:bg-red-900/20 hover:text-red-400 hover:border-red-900/30",
 ...
 className="hidden lg:flex absolute -right-3 top-20 w-6 h-6 bg-black/70 backdrop-blur-md border border-border rounded-full items-center justify-center shadow-sm hover:border-primary/40 transition-colors z-50"
 >
 ...
 return (
 <>
 <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-black/70 backdrop-blur-xl border-b border-border p-4 hide-on-print">
 <div className="flex items-center justify-between">
 <Link href="/" className="flex items-center space-x-2">
 <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center">
 <span className="text-white font-bold">SH</span>
 </div>
 <span className="font-bold text-white text-xs">SmartHotel</span>
 </Link>
 <Button
 variant="ghost"
 size="icon"
 ...
 <div
 className="fixed inset-y-0 left-0 w-72 bg-card border-r border-border shadow-sm flex flex-col animate-in slide-in-from-left duration-300"
 ...
 <aside className={cn(
 "hidden lg:flex fixed left-0 top-0 bottom-0 bg-card border-r border-border flex-col transition-all duration-300 z-40 hide-on-print",
