...
const STATUS_CONFIG: Record<string, { label: string, color: string, badge: string, icon: any }> = {
 AVAILABLE: { label: "Available", color: "bg-emerald-950/20 border-border", badge: "text-emerald-400 border-border bg-emerald-500/10", icon: CheckCircle2 },
 OCCUPIED: { label: "Occupied", color: "bg-rose-950/40 border-border", badge: "text-rose-400 border-border bg-rose-500/10", icon: UserCheck },
 DIRTY: { label: "Dirty", color: "bg-amber-950/40 border-border", badge: "text-amber-400 border-border bg-amber-500/10", icon: Clock },
 ...
 MAINTENANCE: { label: "Maintenance", color: "bg-slate-900/60 border-slate-700/40", badge: "text-muted-foreground border-slate-700/20 bg-slate-700/10", icon: Hammer },
 BLOCKED: { label: "Blocked", color: "bg-black/60 border-border", badge: "text-muted-foreground border-border bg-muted/50", icon: Ban },
}
...
export function RoomStatusGrid({
 ...
 <CardDescription className="text-muted-foreground text-xs mt-1">Operational floor-map & state intelligence</CardDescription>
 </div>
 </div>
 ...
 className="bg-muted/50 border border-border text-xs text-white rounded-lg pl-12 pr-4 py-4 w-full focus:outline-none focus:ring-1 focus:ring-primary/50 placeholder-white/20 transition-all shadow-inner-light"
 />
 </div>
 
 <div className="flex items-center gap-1 bg-black/40 p-1.5 border border-border rounded-lg">
 <div className="px-3 text-xs font-bold text-muted-foreground hidden md:block">Filter</div>
 ...
 className={cn(
 "text-xs tracking-[0.1em] font-bold px-3 py-2.5 rounded-xl transition-all whitespace-nowrap",
 statusFilter === st ? 'bg-primary text-white shadow-lg' : 'text-muted-foreground hover:text-white/60 hover:bg-muted/50'
 )}
 >
 ...
 <div className="flex items-center gap-4">
 <h2 className="text-sm font-bold text-muted-foreground whitespace-nowrap">Floor {floor}</h2>
 ...
 <p className="text-muted-foreground text-xs font-bold mt-2">{room.type}</p>
 </div>
 {room.vip && (
 <div className="p-1.5 rounded-lg bg-amber-500/10 border border-border">
