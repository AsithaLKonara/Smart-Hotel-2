...
export default function TapeChart({ initialStartDate = new Date(), daysToShow = 14 }: TapeChartProps) {
 ...
 return (
 <div className="w-full bg-[#0a0a0a] border border-purple-900/30 overflow-hidden relative shadow-sm flex flex-col font-sans">
 ...
 <button 
 className="px-3 py-1 bg-muted/50 border border-purple-900/30 text-purple-300 rounded text-sm hover:bg-white/10"
 ...
 <span className="text-xs text-muted-foreground font-bold tracking-wider">Rooms</span>
 </div>
 {rooms.map(room => (
 ...
 {rooms.map(room => (
 <div key={room.id} className="flex border-b border-purple-900/10 hover:bg-muted/50" style={{ height: ROW_HEIGHT }}>
 {dateHeaders.map((date, idx) => (
 <div
 key={idx}
