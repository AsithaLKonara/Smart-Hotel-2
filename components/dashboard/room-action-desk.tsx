...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
 AVAILABLE: { label: 'Available', color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-border', dot: 'bg-emerald-400' },
 OCCUPIED: { label: 'Occupied', color: 'text-rose-400', bg: 'bg-rose-500/10 border-border', dot: 'bg-rose-400' },
 CLEANING: { label: 'Cleaning', color: 'text-amber-400', bg: 'bg-amber-500/10 border-border', dot: 'bg-amber-400' },
 MAINTENANCE: { label: 'Maintenance', color: 'text-muted-foreground', bg: 'bg-slate-500/10 border-slate-500/20', dot: 'bg-slate-400' },
...
export function RoomActionDesk({
 ...
 status: 'AVAILABLE',
 icon: CheckCircle,
 style: 'border-border bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300',
 ...
 status: 'OCCUPIED',
 icon: BedDouble,
 style: 'border-border bg-rose-500/10 hover:bg-rose-500/20 text-rose-300',
 ...
 status: 'CLEANING',
 icon: Sparkles,
 style: 'border-border bg-amber-500/10 hover:bg-amber-500/20 text-amber-300',
 ...
 {/* Modal */}
 <div className="relative w-full max-w-3xl max-h-[95vh] flex flex-col bg-[#0d0d0d] border border-border rounded-xl shadow-[0_40px_140px_-20px_rgba(0,0,0,0.9)] overflow-hidden">

 {/* Gold top line */}
 <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-primary/70 to-transparent" />

 {/* ─── HEADER ─── */}
 <div className="flex items-center justify-between p-6 border-b border-border bg-muted/50 flex-shrink-0">
 ...
 <button
 onClick={onClose}
 className="w-10 h-10 flex items-center justify-center rounded-xl border border-border text-muted-foreground hover:text-white hover:bg-white/10 transition-all"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 : 'text-muted-foreground hover:text-white/60 border-b-2 border-transparent'
 )}
 >
 ...
 <div className={cn(
 'rounded-lg p-5 border flex flex-col sm:flex-row sm:items-center justify-between gap-4',
 room.status === 'AVAILABLE'
 ? 'bg-emerald-500/10 border-border'
 : 'bg-primary/10 border-primary/20'
 )}>
 <div>
 <p className={cn('text-xs font-bold mb-0.5', room.status === 'AVAILABLE' ? 'text-emerald-400' : 'text-primary')}>
 ...
 <div>
 <p className="text-xs text-muted-foreground font-bold mb-3 flex items-center gap-2">
 ...
 <div>
 <p className="text-xs text-muted-foreground font-bold mb-3">Billing & Payments</p>
 ...
 className="flex items-center gap-3 p-4 rounded-lg border border-white/8 bg-white/[0.03] hover:bg-white/[0.07] text-white/60 hover:text-white transition-all text-left"
 >
 ...
 {/* Current guest */}
 {currentBooking ? (
 <div className="rounded-lg bg-emerald-500/10 border border-border p-5">
 <p className="text-xs font-bold text-emerald-400 mb-3">Current Guest</p>
 <div className="flex items-center gap-4">
 ...
 <p className="text-muted-foreground text-sm">{currentBooking.user?.email}</p>
 ...
 <p className="text-xs text-emerald-400/60 font-semibold mb-2">Send Email</p>
 ...
 className="px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-border text-emerald-300 text-[11px] font-semibold hover:bg-emerald-500/20 transition-all"
 >
 ...
 <div className="rounded-lg bg-white/[0.03] border border-white/8 p-5 text-center">
 <User className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
 <p className="text-muted-foreground text-sm font-medium">No guest currently in this room</p>
 </div>
 )}
 ...
 isVip
 ? 'bg-amber-500/10 border-border shadow-lg shadow-amber-950/20'
 : 'bg-white/[0.03] border-white/8 hover:border-white/15'
 )}
 >
 <div className={cn(
 'w-12 h-12 rounded-lg flex items-center justify-center transition-all flex-shrink-0',
 isVip ? 'bg-amber-500 text-black' : 'bg-white/8 text-muted-foreground'
 )}>
 ...
 <div key={booking.id} className="flex items-center gap-3 p-4 rounded-lg bg-white/[0.03] border border-white/8 hover:bg-white/[0.05] transition-all group">
 ...
 {/* Notes */}
 <div>
 <label className="text-xs text-muted-foreground font-bold mb-2 flex items-center gap-2 ml-1">
 ...
 rows={4}
 className="w-full bg-white/[0.03] border border-border text-sm text-white p-4 focus:outline-none focus:ring-2 focus:ring-primary/40 rounded-lg placeholder-white/20 resize-none transition-all leading-relaxed"
 />
 <p className="text-xs text-muted-foreground mt-1 ml-1">Saved automatically when you click away</p>
 </div>
 </div>
 )}
 ...
 {room.status !== 'AVAILABLE' ? (
 <div className="rounded-lg bg-amber-500/10 border border-border p-5 flex gap-3">
 ...
 className="w-full flex justify-center items-center gap-2 p-3 bg-muted/50 border border-border hover:bg-white/10 rounded-xl text-sm text-white transition-all mb-4"
 >
 ...
 {/* Guest Name */}
 <div>
 <label className="text-xs text-muted-foreground font-semibold tracking-wider ml-1 mb-1.5 block">Guest Name *</label>
 ...
 className="w-full bg-muted/50 border border-border h-12 pl-11 pr-4 rounded-xl text-sm text-white placeholder-white/20 focus:ring-2 focus:ring-primary/40 focus:outline-none transition-all"
 />
 </div>
 </div>
 ...
 {/* Phone */}
 <div>
 <label className="text-xs text-muted-foreground font-semibold tracking-wider ml-1 mb-1.5 block">Phone Number</label>
 <div className="relative">
 ...
 {/* Email */}
 <div>
 <label className="text-xs text-muted-foreground font-semibold tracking-wider ml-1 mb-1.5 block">Email <span className="normal-case text-muted-foreground">(optional)</span></label>
 <div className="relative">
 ...
 {/* Nights */}
 <div>
 <label className="text-xs text-muted-foreground font-semibold tracking-wider ml-1 mb-1.5 block">How Many Nights?</label>
 <div className="relative">
 ...
 className="w-full bg-muted/50 border border-border h-12 pl-11 pr-4 rounded-xl text-sm text-white focus:ring-2 focus:ring-primary/40 focus:outline-none appearance-none transition-all"
 >
