...
export function GuestRequests() {
 ...
 <p className="text-xs text-muted-foreground font-medium">Housekeeping, Maintenance & Butler requests</p>
 </div>
 <Button 
 ...
 <p className="text-sm text-muted-foreground font-light">No active requests.</p>
 </div>
 ) : (
 <div className="space-y-4">
 {tasks.slice(0, 3).map((task) => (
 <div key={task.id} className="flex items-center justify-between p-6 bg-white/[0.03] border border-border rounded-xl hover:border-primary/30 transition-all group/item">
 ...
 <Badge variant="outline" className={`text-xs font-bold px-2 py-0.5 border-border ${task.priority === 'HIGH' ? 'text-red-400' : 'text-muted-foreground'}`}>
 {task.priority}
 </Badge>
 </div>
 ))}
