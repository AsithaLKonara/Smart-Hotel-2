...
export function StayJourneyTimeline({ 
 ...
 return (
 <Card className="bg-muted/50 border-border backdrop-blur-xl mb-10 overflow-hidden relative">
 <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-primary/20 to-transparent" />
 <CardContent className="p-8 lg:p-10">
 <h3 className="text-xs font-bold text-muted-foreground mb-10 text-center lg:text-left">Your Experience Journey</h3>

 <div className="relative flex flex-col lg:flex-row justify-between gap-10">
 {/* Connecting Line (Desktop) */}
 <div className="hidden lg:block absolute top-6 left-12 right-12 h-[1px] bg-muted/50 -z-0" />
 ...
 "bg-muted/50 border-border opacity-40 group-hover:opacity-60"
 )}>
 {isPassed ? (
 <CheckCircle className="w-6 h-6 text-emerald-400" />
 ) : (
 <span className={cn("text-sm font-bold font-mono", isActive ? "text-primary" : "text-muted-foreground")}>
 0{idx + 1}
 </span>
 )}
 </div>
 ...
 "mb-2 border-none px-0 text-xs font-bold ",
 isActive ? "text-primary" : isPassed ? "text-emerald-400" : "text-muted-foreground"
 )}>
 {step.label}
 </Badge>
 <h4 className={cn(
