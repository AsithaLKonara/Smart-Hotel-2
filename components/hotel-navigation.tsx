...
export default function HotelNavigation() {
 ...
 <header
 role="banner"
 className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/60 backdrop-blur-xl border-b border-border shadow-sm py-2' : 'bg-transparent py-4'
 }`}
 >
 {/* Top Bar - Hidden on scroll for cleaner look */}
 {!scrolled && (
 <div className="border-b border-border text-white/70 py-2 hidden lg:block">
 ...
 <div className="absolute inset-0 bg-primary/10 rounded-full rotate-45 group-hover:rotate-180 transition-transform duration-700" />
 ...
 <Link href="/booking">
 <Button className="bg-primary/10 hover:opacity-90 text-white px-8 rounded-xl border-none font-serif text-xs h-12 transition-all hover:scale-105 active:scale-95 shadow-sm">
 Book Your Stay
 </Button>
 </Link>
 </div>
 ...
 <Button className="w-full bg-primary/10 text-white py-8 text-lg font-serif rounded-xl">
 Book Now
 </Button>
 </Link>
 </div>
