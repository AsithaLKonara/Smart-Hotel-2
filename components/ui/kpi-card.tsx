...
export function KpiCard({
 ...
 isAnomaly && "border-border ring-1 ring-rose-500/20",
 ...
 <h3 className="text-xs font-bold text-white/70 tracking-[0.15em]">
 {title}
 </h3>
 </div>
 
 {isAnomaly ? (
 <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-rose-500/10 border border-border text-rose-500 animate-pulse">
 <AlertCircle className="w-3 h-3" />
 <span className="text-xs font-bold ">Anomaly</span>
 </div>
 ) : delta !== undefined && (
 <div className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-muted/50 border border-border", getTrendColor())}>
 {getTrendIcon()}
 ...
 {aiInsight ? (
 <div className="flex items-start gap-1.5 p-1.5 rounded-lg bg-emerald-500/5 border border-border">
 <Sparkles className="w-3 h-3 text-emerald-400 mt-0.5 shrink-0" />
 <p className="text-xs text-emerald-400/80 leading-tight font-medium italic">
 "{aiInsight}"
 </p>
 </div>
 ) : (subtitle || deltaLabel) && (
 ...
 {actionLabel && (
 <div className="mt-4 pt-3 border-t border-border flex items-center justify-between group/action">
 <span className="text-xs font-bold text-white/70 tracking-wider">{actionLabel}</span>
