...
export default function UnifiedPOS({ role }: { role: string }) {
 ...
 return (
 <div className="flex h-[calc(100vh-80px)] overflow-hidden bg-card text-white print-container print:bg-white print:text-black">
 
 {/* 1. LEFT COLUMN: Active Guests */}
 <div className="w-[300px] border-r border-border flex flex-col hide-on-print">
 ...
 className="pl-9 h-9 bg-muted/50 border-border text-white text-sm w-full"
 />
 </div>
 <Button 
 variant="outline" 
 ...
 {bookingsLoading ? (
 <div className="text-center text-muted-foreground text-sm mt-4">Loading guests...</div>
 ) : filteredBookings.length === 0 ? (
 <div className="text-center text-muted-foreground text-sm mt-4">No matching guests.</div>
 ) : (
 ...
 <Card 
 key={booking.id} 
 className={`bg-[#1a1a1a] border ${activeBooking?.id === booking.id ? 'border-primary shadow-sm' : 'border-border'} hover:border-primary/50 cursor-pointer transition-colors`}
 onClick={() => {
 ...
 <Badge variant="outline" className="text-xs text-white/50 border-border">{roomTypeName}</Badge>
 </div>
 <div className="flex items-center gap-2 text-sm text-white/80">
 <User className="w-3 h-3 text-muted-foreground" /> {booking.guest?.name || 'Guest'}
 </div>
 ...
 <div className="flex-1 flex flex-col border-r border-border hide-on-print">
 <div className="p-4 border-b border-border bg-black/20">
 ...
 className="pl-9 h-9 bg-muted/50 border-border text-white text-sm"
 />
 </div>
 </div>
 ...
 className={`h-8 text-xs whitespace-nowrap ${categoryFilter === cat ? 'bg-primary text-white' : 'bg-muted/50 text-white/70 border-border hover:bg-white/10 hover:text-white'}`}
 >
 {cat}
 </Button>
 ))}
 ...
 <Card 
 key={item.id} 
 className="bg-[#1a1a1a] border-border hover:border-primary/50 transition-colors cursor-pointer"
 onClick={() => {
 addToCart(item)
 setActiveTab('cart')
 }}
 ...
 <div className="p-3 border-b border-border bg-black/30 flex justify-between items-center hide-on-print">
 <div className="flex bg-muted/50 p-1 rounded-xl border border-border">
 <button
 onClick={() => setActiveTab('cart')}
 className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'cart' ? 'bg-primary text-white shadow-md' : 'text-muted-foreground hover:text-white'}`}
 >
 <ShoppingCart className="w-3.5 h-3.5" /> Cart ({posCart.reduce((sum, i) => sum + i.quantity, 0)})
 </button>
 <button
 onClick={() => setActiveTab('folio')}
 className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'folio' ? 'bg-primary text-white shadow-md' : 'text-muted-foreground hover:text-white'}`}
 >
 ...
 <div className="p-4 border-b border-border bg-black/20 flex justify-between items-center hide-on-print">
 <div>
 <p className="text-xs font-bold tracking-wider text-muted-foreground">Selected Customer</p>
 ...
 {activeBooking && (
 <span className="text-xs bg-emerald-500/20 text-emerald-400 border border-border px-2 py-0.5 rounded font-bold ">
 CHECKED-IN
 </span>
 )}
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-1">Select items from the catalog on the left to build an order.</p>
 </div>
 ) : (
 posCart.map((item, i) => (
 <div key={i} className="bg-muted/50 border border-border p-3 rounded-xl flex justify-between items-center">
 <div className="flex-1 pr-2">
 <p className="text-sm font-semibold text-white">{item.name}</p>
 <p className="text-xs text-muted-foreground">${Number(item.price || 0).toFixed(2)} each</p>
 </div>
 <div className="flex items-center gap-3">
 <div className="flex items-center gap-1.5 bg-black/40 px-2 py-1 rounded-lg border border-border">
 ...
 <p className="text-xs text-muted-foreground mt-1">Select an active checked-in guest from the left sidebar to view their live folio statement.</p>
 </div>
 ) : (
 <div className="flex-1 flex flex-col overflow-hidden">
 {/* Folio Summary Totals */}
 <div className="p-4 bg-muted/50 border-b border-border grid grid-cols-3 gap-2 text-center">
 <div className="bg-black/40 p-2.5 rounded-lg border border-border">
 <p className="text-xs font-bold text-muted-foreground">Total Charges</p>
 ...
 {/* Line Items List */}
 <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
 <h3 className="text-xs font-bold tracking-wider text-muted-foreground flex items-center justify-between">
 <span>Itemized Folio Charges</span>
 <span className="text-xs text-muted-foreground font-mono">Folio #{activeFolio?.id?.substring(0,8).toUpperCase()}</span>
 </h3>
 ...
 <div key={item.id} className="bg-muted/50 border border-border p-3 rounded-xl flex justify-between items-center">
 <div>
 <p className="text-xs font-semibold text-white">{item.description}</p>
 <span className="text-xs bg-white/10 px-1.5 py-0.5 rounded text-slate-300 font-bold mt-1 inline-block">
 {item.category}
 </span>
 </div>
 ...
 <div className="p-4 border-t border-border bg-black/40 space-y-3">
 <div>
 <label className="block text-xs font-bold tracking-wider text-muted-foreground mb-1">
 ...
 className="bg-muted/50 border-border text-white font-mono text-xs h-9"
 />
 </div>
 ...
 <DialogContent className="bg-[#121212] border-border text-white max-w-sm rounded-lg p-6">
 ...
 <h1 className="text-sm font-bold tracking-wider">SmartHotel Experience Elite</h1>
 <p className="text-xs text-gray-600">POS & Folio Billing Statement</p>
 <p className="text-xs text-gray-500 mt-1">{lastTransaction?.date || new Date().toLocaleString()}</p>
 </div>
 ...
 <div className="flex justify-between font-bold text-xs text-gray-600">
 <span>Item</span>
 ...
 <div className="text-center pt-4 border-t border-dashed border-gray-400 text-xs text-gray-500">
 ...
 <Button variant="ghost" onClick={() => setPrintReceiptModal(false)} className="text-xs text-muted-foreground hover:text-white">
 Close
 </Button>
