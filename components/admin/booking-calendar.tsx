...
export function BookingCalendar({ onMutationSuccess }: BookingCalendarProps) {
 ...
 switch (status) {
 case 'CONFIRMED':
 return 'bg-emerald-500/20 text-emerald-400 border border-border'
 case 'CHECKED_IN':
 ...
 case 'PENDING':
 default:
 return 'bg-amber-500/20 text-amber-400 border border-border'
 }
 }
 ...
 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-muted/50 backdrop-blur-md p-4 border border-border rounded-lg">
 <div className="flex items-center space-x-3">
 <Button onClick={prevMonth} variant="outline" size="icon" className="bg-muted/50 border-border text-white hover:bg-white/10 rounded-xl h-9 w-9">
 <ChevronLeft className="w-4 h-4" />
 </Button>
 <div className="text-lg font-serif font-bold text-white min-w-[140px] text-center">
 {currentDate.toLocaleDateString('default', { month: 'long', year: 'numeric' })}
 </div>
 <Button onClick={nextMonth} variant="outline" size="icon" className="bg-muted/50 border-border text-white hover:bg-white/10 rounded-xl h-9 w-9">
 ...
 variant="outline" 
 size="sm" 
 className="bg-muted/50 border-border text-xs text-purple-300 hover:bg-purple-950/20 hover:text-white rounded-xl h-9"
 >
 Today
 </Button>
 </div>
 ...
 className="w-full bg-muted/50 border border-border text-xs text-white pl-10 pr-4 py-2.5 rounded-xl placeholder-white/30 focus:outline-none focus:ring-1 focus:ring-purple-500 focus:bg-slate-950/60"
 />
 </div>
 ...
 className="w-full sm:w-44 bg-slate-950 border border-border text-xs text-slate-300 p-2.5 rounded-xl focus:outline-none focus:ring-1 focus:ring-purple-500 cursor-pointer"
 >
 ...
 {/* Main Grid Calendar */}
 {loading ? (
 <div className="h-[480px] bg-muted/50 border border-border rounded-lg flex items-center justify-center backdrop-blur-md">
 <PremiumSpinner size="lg" text="Syncing PMS calendars..." />
 </div>
 ) : (
 <div className="bg-muted/50 border border-border rounded-lg overflow-hidden backdrop-blur-md shadow-sm">
 {/* Calendar Day Header */}
 <div className="grid grid-cols-7 border-b border-border bg-slate-950/25">
 {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day) => (
 <div key={day} className="text-center py-3 text-xs sm:text-xs font-semibold text-muted-foreground border-r border-border last:border-r-0">
 {day}
 </div>
 ))}
 </div>
 ...
 {/* Calendar Grid cells */}
 <div className="grid grid-cols-7 divide-x divide-y divide-white/5 bg-background/30 min-h-[460px]">
 {gridDays.map((day, idx) => {
 if (day === null) {
 return (
 <div key={`empty-${idx}`} className="p-2 min-h-[90px] sm:min-h-[110px] bg-muted/30 opacity-25 border-r border-b border-border" />
 )
 }
 ...
 className={`p-2 min-h-[90px] sm:min-h-[110px] border-r border-b border-border flex flex-col justify-between transition-colors hover:bg-muted/50 cursor-pointer ${todayMark ? 'bg-purple-950/15 border-purple-500/40' : ''}`}
 >
 <div className="flex items-center justify-between">
 <span className={`text-xs sm:text-xs font-mono font-bold ${todayMark ? 'bg-primary text-white h-5 w-5 sm:h-6 sm:w-6 rounded-full flex items-center justify-center shadow-lg shadow-purple-950' : 'text-white/60'}`}>
 {day.getDate()}
 </span>
 {dateBookings.length > 0 && (
 <Badge className="bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs px-1 py-0 sm:py-0.5">
 {dateBookings.length}
 </Badge>
 )}
 </div>
 ...
 className={`text-xs sm:text-xs px-1.5 py-0.5 rounded truncate font-medium flex items-center justify-between hover:scale-[1.02] transition-transform ${getStatusStyle(b.status)}`}
 >
 <span className="truncate mr-1 max-w-[50px] sm:max-w-[80px]">
 {b.user?.name || "Guest"}
 </span>
 <span className="font-mono text-xs sm:text-xs opacity-80 shrink-0">
 #{b.room?.number || "N/A"}
 </span>
 </div>
 ))}
 {dateBookings.length > 3 && (
 <div className="text-xs sm:text-xs text-muted-foreground text-center font-bold">
 ...
 <Card className="w-full max-w-xl bg-slate-950/90 border border-border rounded-lg shadow-sm max-h-[80vh] overflow-y-auto">
 ...
 <CardDescription className="text-muted-foreground text-xs">A total of {selectedDayBookings.bookings.length} guests booked for stay.</CardDescription>
 </div>
 <Button onClick={() => setSelectedDayBookings(null)} variant="ghost" size="icon" className="text-muted-foreground hover:text-white hover:bg-muted/50 rounded-xl">
 <X className="w-5 h-5" />
 </Button>
 </CardHeader>
 ...
 className="p-4 bg-muted/50 border border-border hover:border-primary/40 rounded-xl transition-all cursor-pointer flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
 >
 <div className="space-y-1">
 <div className="flex items-center gap-2">
 <h4 className="font-bold text-white text-sm">{b.user?.name || "Guest"}</h4>
 <Badge className={`text-xs py-0.5 px-1.5 font-bold tracking-wider ${getStatusStyle(b.status)}`}>
 {b.status.replace('_', ' ')}
 </Badge>
 </div>
 ...
 <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 border-border pt-2 sm:pt-0">
 <div className="text-right sm:mr-2">
 <p className="text-xs text-muted-foreground font-mono">TOTAL CHARGE</p>
 ...
 <Card className="w-full max-w-2xl bg-[#0e0a1b]/95 border border-border rounded-lg shadow-sm relative overflow-hidden">
 {/* Design header lines */}
 ...
 <CardHeader className="border-b border-border pb-4 flex flex-row items-center justify-between bg-slate-950/20">
 <div className="space-y-1">
 <Badge className="bg-primary/15 text-primary border border-primary/20 text-xs font-bold px-2 py-0.5">
 ...
 <CardDescription className="text-muted-foreground text-xs">Verify booking property credentials, billing limits, and guest preferences.</CardDescription>
 </div>
 <Button onClick={() => setSelectedBooking(null)} variant="ghost" size="icon" className="text-muted-foreground hover:text-white hover:bg-muted/50 rounded-xl absolute top-4 right-4">
 <X className="w-5 h-5" />
 </Button>
 </CardHeader>
 ...
 <h4 className="text-xs font-bold text-muted-foreground flex items-center gap-1.5 border-b border-border pb-2">
 <User className="w-3.5 h-3.5 text-primary" /> Guest Identification
 </h4>
 <div className="space-y-2 text-xs">
 <div className="flex items-center justify-between p-2.5 bg-muted/50 border border-border rounded-xl">
 ...
 <div className="flex items-center justify-between p-2.5 bg-muted/50 border border-border rounded-xl">
 <span className="text-muted-foreground">Guests Quantity</span>
 ...
 <div className="flex items-center gap-1.5 text-xs font-bold tracking-wider text-purple-400">
 ...
 <Badge className={`text-xs tracking-wider font-bold py-0.5 px-2 ${getStatusStyle(selectedBooking.status)}`}>
 ...
 variant="outline"
 className="border-border text-white/80 hover:text-white text-xs h-9 font-semibold hover:bg-muted/50 rounded-xl"
 >
 Set Pending
 </Button>
 <Button
 ...
 className="bg-emerald-950/30 border border-border hover:bg-emerald-600 hover:text-white text-emerald-400 text-xs h-9 font-bold rounded-xl"
 >
 Confirm Booking
 </Button>
 <Button
 ...
 className="w-full bg-rose-950/30 border border-border hover:bg-rose-600 text-rose-300 hover:text-white text-xs h-9 font-bold rounded-xl tracking-wider mt-2"
 >
