...
export function OrderPortal({ roomNumber = "101", guestInfo }: OrderPortalProps) {
 ...
 className={`px-8 py-3 rounded-lg font-bold text-xs transition-all border ${
 isSelected
 ...
 <Card key={i} className="h-64 bg-muted/50 border-border animate-pulse rounded-xl" />
 ))
 ) : filteredItems.length === 0 ? (
 <div className="col-span-full py-20 text-center bg-muted/50 rounded-xl border border-border">
 <UtensilsCrossed className="w-12 h-12 text-white/10 mx-auto mb-4" />
 <p className="text-muted-foreground font-serif italic text-lg">No culinary items found in this collection.</p>
 </div>
 ) : (
 ...
 key={item.id}
 >
 <Card className="bg-card border-border rounded-xl overflow-hidden group hover:border-primary/30 transition-all duration-500 h-full flex flex-col shadow-sm">
 {item.image && (
 ...
 <Badge className="absolute top-6 left-6 bg-black/60 backdrop-blur-md border-border text-xs font-bold px-3 py-1">
 {item.category}
 </Badge>
 </div>
 )}
 ...
 <span className="text-xs font-bold ">{item.preparationTime} min Prep</span>
 </div>
 )}
 </div>
 ...
 className="bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 rounded-lg h-12 px-6 flex items-center gap-2 text-xs font-bold "
 >
 ...
 <Card className="bg-card border-border rounded-xl p-10 space-y-8 shadow-sm">
 <div className="flex items-center justify-between">
 <div className="space-y-1">
 <p className="text-xs font-bold text-muted-foreground ">Your Selection</p>
 ...
 {cart.length === 0 ? (
 <div className="py-12 text-center space-y-4 bg-muted/50 rounded-xl border border-border">
 <ShoppingCart className="w-10 h-10 text-white/10 mx-auto" />
 <p className="text-sm text-muted-foreground font-light">Your palette is currently empty.</p>
 </div>
 ) : (
 <>
 <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
 {cart.map(({ item, quantity }) => (
 <div key={item.id} className="flex items-center justify-between p-4 bg-white/[0.03] border border-border rounded-lg group/cart transition-colors hover:border-border">
 <div className="flex-1">
 <p className="font-bold text-sm text-white">{item.name}</p>
 <p className="text-xs text-muted-foreground ">${Number(item.price || 0).toFixed(2)} each</p>
 </div>
 <div className="flex items-center gap-3 bg-black/40 rounded-xl p-1.5 border border-border">
 <button
 onClick={() => handleUpdateQuantity(item.id, -1)}
 className="w-8 h-8 flex items-center justify-center bg-muted/50 rounded-lg hover:bg-white/10 text-white/60 transition-colors"
 >
 ...
 <span className="text-muted-foreground">Subtotal Service</span>
 ...
 className="w-full h-16 bg-primary hover:bg-primary/90 text-white rounded-lg text-xs font-bold shadow-sm transition-all active:scale-95"
 >
 {isSubmitting ? (
