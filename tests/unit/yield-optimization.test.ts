...
describe('YieldOptimizationEngine Unit Tests', () => {
  ...
  describe('calculateDemandElasticity', () => {
    ...
    it('should return correct elasticity for different occupancy ranges', async () => {
  ...
  describe('calculateDynamicPrice', () => {
    ...
    it('should apply early bird discount for booking far in advance', async () => {
      const result = await YieldOptimizationEngine.calculateDynamicPrice(100, 0.5, 35, 'weekday')
      expect(result.multiplier).toBe(0.9)
      expect(result.appliedRules).toContain('EARLY_BIRD_INCENTIVE')
    })

    it('should apply last minute premium for high occupancy stays close to arrival', async () => {
      const result = await YieldOptimizationEngine.calculateDynamicPrice(100, 0.7, 2, 'weekday')
      // base multiplier = 1.0
    ...
    it('should apply weekend premium', async () => {
      const result = await YieldOptimizationEngine.calculateDynamicPrice(100, 0.5, 10, 'weekend')
      expect(result.multiplier).toBe(1.1)
      expect(result.appliedRules).toContain('WEEKEND_PREMIUM')
    })

    it('should cap the multiplier at 0.50 minimum', async () => {
      ...
      const result = await YieldOptimizationEngine.calculateDynamicPrice(100, 0.1, 40, 'weekday')
  ...
  describe('calculateOverbookingBuffer', () => {
    ...
    it('should calculate safe buffer and risk rating correctly', async () => {
