...
function BookingPageContent() {
 ...
 const inputClass = "w-full bg-muted/50 border border-border text-white placeholder:text-muted-foreground px-5 py-4 rounded-xl text-sm focus:ring-1 focus:ring-primary transition-all"
 ...
 {/* Contextual Header */}
 <section className="relative pt-40 pb-24 overflow-hidden min-h-[45vh] flex flex-col justify-end border-b border-border">
 <Image 
 ...
 <div className="flex items-center justify-center space-x-3 text-primary tracking-[0.4em] text-xs font-bold">
 ...
 <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 border ${step >= s ? 'bg-primary/10 border-primary/50 text-white shadow-sm' : 'bg-muted/50 border-border text-muted-foreground'}`}>
 <Icon className="w-5 h-5" />
 </div>
 <span className={`text-xs font-bold ${step >= s ? 'text-primary' : 'text-muted-foreground'}`}>{t}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 >
 <div className="lg:col-span-6 relative aspect-[4/3] overflow-hidden rounded-lg shadow-sm">
 ...
 <div className="lg:col-span-6 bg-black/40 backdrop-blur-xl border border-border rounded-lg p-8 lg:p-12 shadow-sm space-y-8">
 <div className="space-y-3">
 <h2 className="text-3xl font-serif font-bold text-white">Plan Your Arrival</h2>
 <p className="text-muted-foreground font-light text-sm">Select your preferred dates and guest count to view available luxury accommodations.</p>
 </div>
 ...
 <Button onClick={searchRooms} disabled={isSearching || !searchData.checkIn || !searchData.checkOut} className="w-full bg-primary/10 text-white h-14 rounded-xl tracking-[0.2em] text-xs font-bold border-none shadow-sm hover:opacity-90">
 ...
 <p className="text-muted-foreground text-sm">{nights} Night{nights !== 1 ? 's' : ''} · {searchData.guests} Guest{searchData.guests !== 1 ? 's' : ''}</p>
 </div>
 <Button variant="ghost" onClick={() => setStep(1)} className="text-primary text-xs font-bold hover:text-primary/80">
 ← Modify Dates
 </Button>
 </div>
 ...
 <h3 className="text-2xl font-serif text-muted-foreground">No suites available for these dates.</h3>
 <Button onClick={() => setStep(1)} className="bg-primary/10 text-white rounded-xl px-8 h-12 text-xs font-bold border-none">Try Different Dates</Button>
 </div>
 ) : (
 <div className="relative">
 {/* Carousel arrows */}
 {carouselIdx > 0 && (
 <button onClick={() => scrollCarousel('prev')} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-6 z-20 w-12 h-12 bg-black/60 backdrop-blur-md border border-border rounded-full flex items-center justify-center text-white hover:border-primary/40 hover:text-primary transition-all">
 <ChevronLeft className="w-5 h-5" />
 </button>
 )}
 {carouselIdx < availableRooms.length - 1 && (
 <button onClick={() => scrollCarousel('next')} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-6 z-20 w-12 h-12 bg-black/60 backdrop-blur-md border border-border rounded-full flex items-center justify-center text-white hover:border-primary/40 hover:text-primary transition-all">
 ...
 <div key={room.id} className={`flex-none w-[85%] md:w-[48%] snap-center group bg-muted/50 backdrop-blur-md border rounded-lg overflow-hidden transition-all duration-300 ${idx === carouselIdx ? 'border-primary/30 shadow-sm' : 'border-border'}`}>
 ...
 <div className="space-y-1">
 <p className="text-xs text-primary font-bold">Floor {room.floor}</p>
 ...
 <div className="flex gap-4 text-muted-foreground text-xs tracking-wider font-bold">
 ...
 <Button onClick={() => setDetailsModalRoom(room)} variant="outline" className="flex-1 border-white/20 text-white rounded-xl h-11 text-xs font-bold hover:bg-white/10 transition-all">
 View Details
 </Button>
 <Button onClick={() => { setSelectedRoom(room); setStep(3) }} className="flex-1 bg-primary/10 text-white rounded-xl h-11 tracking-[0.15em] text-xs font-bold border-none shadow-sm">
 Select Suite
 </Button>
 </div>
 </div>
 ...
 <Button variant="ghost" onClick={() => setStep(2)} className="text-muted-foreground hover:text-white px-0 hover:bg-transparent -mt-4 -mb-4">
 ...
 {/* Auth guard */}
 {!session?.user && (
 <div className="bg-primary/10 border border-primary/20 rounded-lg p-8 space-y-4">
 ...
 <Button onClick={handleAuthRedirect} className="bg-primary/10 text-white rounded-xl px-8 h-12 text-xs font-bold border-none shadow-sm">
 Sign In to Book
 </Button>
 </div>
 )}
 ...
 {/* Guest particulars */}
 <div className="bg-muted/50 backdrop-blur-md border border-border rounded-lg p-8 space-y-6">
 ...
 {/* Special Requests */}
 <div className="bg-muted/50 backdrop-blur-md border border-border rounded-lg p-8 space-y-4">
 ...
 (bookingData.extras as any)[extra.id] ? 'border-primary/40 bg-primary/10' : 'border-border hover:border-white/20 bg-muted/50'
 }`}
 >
 <div>
 ...
 className={`p-6 border rounded-xl cursor-pointer transition-all ${bookingData.paymentMethod === m.id ? 'border-primary/40 bg-primary/10' : 'border-border hover:border-white/20 bg-muted/50'}`}
 >
 <h4 className="font-serif font-bold text-white mb-1">{m.t}</h4>
 <p className="text-muted-foreground text-xs font-light">{m.d}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="bg-muted/50 backdrop-blur-xl border border-border rounded-lg p-8 sticky top-28 space-y-6">
 <h3 className="text-xl font-serif font-bold text-white border-b border-border pb-4">Stay Summary</h3>
 ...
 <span className="text-primary text-xs font-bold">Total Amount</span>
 <span className="text-3xl font-serif font-bold text-primary">{formatPrice(totalAmount)}</span>
 </div>
 <Button onClick={handleCreateBooking} disabled={isLoading} className="w-full bg-primary/10 text-white h-14 rounded-xl tracking-[0.2em] text-xs font-bold border-none shadow-sm hover:opacity-90">
 ...
 <p className="text-muted-foreground text-sm">Please provide your card details to secure the reservation.</p>
 </div>
 ...
 >
 <div className="w-24 h-24 bg-primary/10 border border-primary/20 rounded-lg flex items-center justify-center mx-auto text-primary">
 ...
 <p className="text-muted-foreground font-light max-w-xl mx-auto">Your sanctuary awaits. A confirmation with your booking details has been sent to your inbox.</p>
 </div>
 <div className="flex flex-col sm:flex-row gap-4 justify-center">
 <Button onClick={() => router.push('/my-bookings')} className="bg-primary/10 text-white h-14 rounded-xl px-10 text-xs font-bold border-none shadow-sm">
 Manage Booking
 </Button>
 <Button variant="outline" onClick={() => router.push('/')} className="border-white/20 text-white h-14 rounded-xl px-10 text-xs font-bold hover:bg-white/10">
 Return Home
 </Button>
 </div>
 </motion.div>
 ...
 className="relative w-full max-w-2xl bg-[#0a0a0a] border border-border rounded-xl overflow-hidden shadow-sm z-10 max-h-[90vh] flex flex-col"
 >
 ...
 className="absolute top-4 right-4 w-8 h-8 bg-black/50 hover:bg-black/80 rounded-full flex items-center justify-center text-white backdrop-blur-md transition-colors border border-border"
 >
 ✕
 </button>
 </div>
 ...
 <div>
 <p className="text-primary text-xs font-bold mb-1">Floor {detailsModalRoom.floor}</p>
 ...
 <p className="text-muted-foreground text-xs ">Total for {nights} night{nights !== 1 ? 's' : ''}</p>
 </div>
 </div>
 ...
 <p className="text-muted-foreground text-xs ">WiFi</p>
 </div>
 </div>

 <div className="flex gap-4 pt-4 border-t border-border">
 <Button onClick={() => setDetailsModalRoom(null)} variant="outline" className="flex-1 border-white/20 text-white h-14 rounded-xl text-xs font-bold hover:bg-white/10">
 Close
 </Button>
 <Button 
 onClick={() => { setSelectedRoom(detailsModalRoom); setStep(3); setDetailsModalRoom(null); }} 
 className="flex-1 bg-primary/10 text-white h-14 rounded-xl text-xs font-bold border-none shadow-sm"
 >
 Select Suite
 </Button>
 </div>
