...
function ModalShell({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
 ...
 <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-[#0a0a0f] border border-border rounded-lg shadow-sm p-6 z-50 max-h-[90vh] overflow-y-auto">
...
export default function PurchaseOrdersPage() {
 ...
 <tr>
 <th className="p-4 text-white/50 text-xs ">PO Number</th>
 <th className="p-4 text-white/50 text-xs ">Vendor</th>
 <th className="p-4 text-white/50 text-xs ">Date</th>
 <th className="p-4 text-white/50 text-xs ">Expected</th>
 <th className="p-4 text-white/50 text-xs text-right">Total</th>
 <th className="p-4 text-white/50 text-xs ">Status</th>
 <th className="p-4 text-white/50 text-xs text-right">Actions</th>
 </tr>
 </thead>
 ...
 <div className="w-8 h-8 rounded bg-muted/50 border border-border flex items-center justify-center flex-shrink-0">
 ...
 {orders.length === 0 && (
 <tr><td colSpan={6} className="p-12 text-center text-muted-foreground">No purchase orders found.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Vendor</label>
 <select required value={formData.vendorId} onChange={e => setFormData({ ...formData, vendorId: e.target.value })}
 className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 <option value="">Select vendor...</option>
 {vendors.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
 </select>
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Expected Delivery</label>
 <Input type="date" value={formData.expectedDate} onChange={e => setFormData({ ...formData, expectedDate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div className="col-span-2">
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Notes</label>
 <Input value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="Optional instructions..." />
 </div>
 </div>

 <div className="border border-border rounded-xl p-4 bg-muted/50">
 <h4 className="font-bold text-white mb-4 text-sm tracking-wider">Order Lines</h4>
 {formData.orderItems.map((item, idx) => (
 <div key={idx} className="flex gap-3 mb-3 items-end">
 <div className="flex-1">
 {idx === 0 && <label className="text-xs text-muted-foreground tracking-wider mb-1 block">Item</label>}
 <select required value={item.itemId} onChange={e => handleItemChange(idx, 'itemId', e.target.value)}
 className="w-full bg-[#1a1a24] border border-border rounded-lg p-2 text-sm text-white">
 <option value="">Select item...</option>
 {items.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
 </select>
 </div>
 <div className="w-20">
 {idx === 0 && <label className="text-xs text-muted-foreground tracking-wider mb-1 block">Qty</label>}
 <Input type="number" min="1" required value={item.quantity}
 onChange={e => handleItemChange(idx, 'quantity', e.target.value)}
 className="bg-[#1a1a24] border-border text-white text-center" />
 </div>
 <div className="w-28">
 {idx === 0 && <label className="text-xs text-muted-foreground tracking-wider mb-1 block">Unit Price</label>}
 <Input type="number" step="0.01" required value={item.unitPrice}
 onChange={e => handleItemChange(idx, 'unitPrice', e.target.value)}
 className="bg-[#1a1a24] border-border text-white font-mono text-sm" />
 </div>
 <button type="button" onClick={() => removeItemRow(idx)}
 className="text-muted-foreground hover:text-rose-400 transition-colors mb-1 flex-shrink-0">
 <X className="w-4 h-4" />
 </button>
 </div>
 ))}
 ...
 className="border-border text-white hover:bg-muted/50 mt-2">
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Target Location</label>
 <select required value={receiveForm.targetLocation} onChange={e => setReceiveForm({ ...receiveForm, targetLocation: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Delivery Notes</label>
 <Input value={receiveForm.notes} onChange={e => setReceiveForm({ ...receiveForm, notes: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="Optional notes..." />
 </div>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Invoice Number</label>
 <Input required value={invoiceForm.invoiceNumber} onChange={e => setInvoiceForm({ ...invoiceForm, invoiceNumber: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="INV-..." />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Billed Amount ($)</label>
 <Input type="number" step="0.01" required value={invoiceForm.amount} onChange={e => setInvoiceForm({ ...invoiceForm, amount: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
