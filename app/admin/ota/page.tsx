...
export default function OtaChannelSync() {
 ...
 if (loading) return <div className="min-h-screen bg-background flex items-center justify-center"><PremiumSpinner /></div>
 ...
 <Badge className="bg-purple-600/10 hover:bg-purple-600/20 text-purple-400 border border-purple-500/30 text-xs tracking-wider font-bold py-1 px-3">
 OTA INTEGRATION HUB
 </Badge>
 ...
 <p className="text-muted-foreground text-sm mt-1">Multi-channel inventory locks, automated rate markup broadcasters, and live sync webhook telemetry streams.</p>
 </div>
 <div className="flex items-center gap-3">
 ...
 <div className="flex bg-muted/50 p-1 rounded-md border border-purple-900/50">
 <Button 
 ...
 <Card className="bg-muted/50 border border-purple-900/30 backdrop-blur-md rounded-none shadow-xl">
 ...
 <CardDescription className="text-muted-foreground text-xs">Real-time mapping health of OTA partners</CardDescription>
 </CardHeader>
 <CardContent className="pt-4 space-y-4">
 {channels.map(ch => (
 <div key={ch.id} className="p-4 bg-muted/30 border border-slate-800 flex items-center justify-between gap-4">
 <div>
 <h4 className="font-bold text-slate-200 text-sm flex items-center gap-1.5">
 <span className={`w-2 h-2 rounded-full ${ch.status === "SYNCED" ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'}`} />
 {ch.name}
 </h4>
 <p className="text-xs text-muted-foreground mt-1 tracking-wider">Mapped: <strong className="text-slate-300">{ch.mappedRooms} Inventory Slots</strong></p>
 </div>
 <div className="text-right">
 <Badge className={`text-xs tracking-wider font-bold px-1.5 py-0.5 ${ch.status === "SYNCED" ? 'text-emerald-400 bg-emerald-500/10 border-border' : 'text-amber-400 bg-amber-500/10 border-border'}`}>
 {ch.status}
 </Badge>
 <p className="text-xs text-muted-foreground mt-1">Ping: <strong className="text-muted-foreground">{ch.latencyMs}ms</strong></p>
 </div>
 </div>
 ))}
 </CardContent>
 ...
 <span className="text-muted-foreground">[{new Date(lg.createdAt).toLocaleTimeString()}]</span>
 <Badge className="bg-muted/50 border border-slate-800 text-slate-300 text-xs rounded-xs font-mono py-0 px-1.5">{lg.direction}</Badge>
 <span className={`font-bold ${lg.status === 'SUCCESS' ? 'text-emerald-400' : 'text-rose-400'}`}>{lg.status}</span>
 </div>
 <p className="text-muted-foreground mt-1 pl-1 truncate">{lg.errorMessage || JSON.stringify(lg.payload)}</p>
 </div>
 )) : <p className="text-muted-foreground italic">No sync logs available.</p>}
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 <CardDescription className="text-muted-foreground text-xs">Link your internal room types to OTA Room Type IDs from Channex/Beds24.</CardDescription>
 </CardHeader>
 <CardContent className="pt-4">
 <table className="w-full text-left text-sm border-collapse">
 <thead>
 <tr className="border-b border-purple-900/30 text-muted-foreground text-xs ">
 ...
 <Button variant="ghost" size="sm" className="text-xs text-muted-foreground hover:text-white">Reset</Button>
 </td>
 </tr>
 )
 })}
