...
export default function ExecutiveIntelligenceDashboard() {
 ...
 <span className="text-xs tracking-[0.25em] text-[#D4AF37] font-medium">OLAP Analytical Engine</span>
 ...
 <div className="lg:col-span-4 bg-[#121118]/85 border border-white/[0.04] rounded-lg p-6 space-y-4">
 <h2 className="text-lg font-serif font-light text-white tracking-wider flex items-center gap-2">
 ...
 {anomalies.map((anom) => (
 <div key={anom.id} className="p-4 rounded-xl bg-muted/30 border border-white/[0.03] space-y-2 relative group">
 <div className="flex justify-between items-center">
 <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
 anom.severity === 'CRITICAL' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-amber-500/10 text-amber-400 border border-border'
 }`}>
 {anom.severity}
 </span>
 <button
 ...
 <span className="text-xs text-[#8E8C94] font-mono block">Flagged: {anom.flaggedAt} • ID: {anom.id}</span>
 </div>
 ))}
