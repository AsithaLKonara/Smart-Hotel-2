...
export default function MaintenanceTickets() {
 ...
 return (
 <Card key={ticket.id} className={`bg-[#1a1a1a] border-border ${statusStr === 'resolved' ? 'opacity-50' : ''}`}>
 ...
 {ticket.room && (
 <Badge variant="outline" className="bg-muted/50">Room {ticket.room.number}</Badge>
 )}
 ...
 className="bg-black/40 border-border text-white text-sm"
 required
 />
 </div>
 ...
 className="w-full px-3 py-2 bg-black/40 border border-border rounded-lg text-white text-sm"
 >
 ...
 <Button type="button" variant="ghost" onClick={() => setShowModal(false)} className="text-muted-foreground">
 Cancel
 </Button>
