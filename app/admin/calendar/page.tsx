...
export default function RoomOccupancyTimelineMatrix() {
 ...
 if (channel === 'AIRBNB') return 'bg-rose-500/20 hover:bg-rose-500/30 text-rose-400 border-border'
 if (channel === 'EXPEDIA') return 'bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border-border'
 ...
 return (
 <div className="min-h-screen bg-background text-slate-100 p-6 font-sans">
 ...
 <Badge className="bg-purple-600/10 text-purple-400 border border-purple-500/30 text-xs tracking-wider font-bold py-1 px-3">
 ...
 <p className="text-muted-foreground text-sm mt-1">Horizontal hour/day reservation planner, split-stay connectors, and temporary hold countdown locks.</p>
 </div>
 <div className="flex items-center gap-3">
 <Button onClick={() => setTimelineStartDay(prev => Math.max(1, prev - 7))} variant="outline" size="sm" className="bg-muted/50 border-purple-900/50 text-purple-300">
 <ChevronLeft className="w-4 h-4 mr-1" /> Previous Week
 </Button>
 <Button onClick={() => setTimelineStartDay(prev => prev + 7)} variant="outline" size="sm" className="bg-muted/50 border-purple-900/50 text-purple-300">
 ...
 <Card className="bg-muted/50 border border-purple-900/30 backdrop-blur-md rounded-none shadow-xl overflow-hidden">
 
 {/* Header controls bar */}
 <div className="p-5 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-950/40">
 <div className="flex items-center gap-2">
 <Grid className="w-5 h-5 text-purple-400" />
 <span className="text-sm font-bold tracking-wider text-slate-200">Timeline Grid Scheduler</span>
 </div>
 <div className="flex items-center gap-4 text-xs text-muted-foreground">
 <div className="flex items-center gap-2"><span className="w-3 h-3 bg-rose-500/20 border border-border rounded" /> Airbnb</div>
 <div className="flex items-center gap-2"><span className="w-3 h-3 bg-amber-500/20 border border-border rounded" /> Expedia</div>
 ...
 <div className="grid grid-cols-8 border-b border-border font-bold bg-slate-950/20">
 <div className="p-4 border-r border-purple-950 text-xs text-purple-400">Room Details</div>
 {daysRange.map(day => (
 <div key={day} className="p-4 text-center text-xs text-slate-300 border-r border-border">
 Day {day}
 <span className="block text-xs text-muted-foreground mt-0.5 font-normal">May {7 + day}</span>
 </div>
 ))}
 </div>
 ...
 {rooms.map(room => (
 <div key={room.id} className="grid grid-cols-8 hover:bg-muted/30 transition-colors h-[80px]">
 ...
 <span className="text-xs text-muted-foreground tracking-wider ">{room.type}</span>
 </div>
 ...
 <div 
 key={day} 
 className="border-r border-border relative flex items-center justify-center p-1.5"
 >
 {activeRes ? (
 <button
 ...
 >
 <span className="text-xs font-bold block truncate text-slate-200">{activeRes.guest}</span>
 {activeRes.isSplitStay && (
 <Badge className="bg-purple-900/40 text-purple-300 border-purple-500/20 text-xs py-0 px-1 font-bold self-start ">Split Stay</Badge>
 )}
 </button>
 ) : hasHold ? (
 <div className="w-full h-full bg-amber-500/10 border border-border p-2 flex flex-col justify-between text-amber-400 font-mono text-xs select-none animate-pulse">
 ...
 className="w-full h-full rounded border border-dashed border-slate-800 hover:border-purple-500/30 hover:bg-purple-500/[0.02] flex items-center justify-center group/btn text-muted-foreground hover:text-purple-400 transition-all"
 >
 ...
 {/* Hold Lock desk desk */}
 {activeHold && (
 <Card className="bg-gradient-to-b from-amber-950/10 to-transparent border border-border rounded-none shadow-sm animate-fade-in">
 <CardHeader className="border-b border-border">
 <Badge className="bg-amber-500/10 text-amber-400 border border-border text-xs font-bold py-0.5 px-2 self-start">LOCK COUNTER DESK</Badge>
 ...
 <CardDescription className="text-muted-foreground text-xs">Awaiting receptionist finalized confirmation or payment authorization lock.</CardDescription>
 </CardHeader>
 ...
 <div className="p-3.5 bg-slate-950 border border-border rounded-xs flex items-center justify-between text-amber-400 mt-4">
 <span className="font-bold text-xs">Autorelease Lock:</span>
 <span className="font-mono text-base font-bold flex items-center gap-1"><Clock className="w-4 h-4 animate-spin-slow" /> {formatTimer(activeHold.secondsLeft)}</span>
 </div>
 </div>

 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground">Verifying Operator Email</label>
 <input 
 type="email" 
 ...
 <Button onClick={releaseManualHold} variant="outline" className="border-slate-800 text-muted-foreground hover:text-white rounded-none">
 Release Lock
 </Button>
 ...
 {selectedRes && (
 <Card className="bg-gradient-to-b from-purple-950/10 to-transparent border border-purple-500/30 rounded-none shadow-sm animate-fade-in">
 <CardHeader className="border-b border-purple-500/15">
 <Badge className="bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs font-bold py-0.5 px-2 self-start">RESERVATION DESK</Badge>
 <CardTitle className="text-xl font-serif text-white mt-1.5 flex items-center gap-2">
 <CheckCircle2 className="w-5 h-5 text-purple-400" /> Inspect Reservation
 </CardTitle>
 <CardDescription className="text-muted-foreground text-xs">Verify credentials and trigger override releases for double bookings.</CardDescription>
 </CardHeader>
 <CardContent className="pt-5 space-y-4">
 
 <div className="p-4 bg-muted/30 border border-slate-800 text-xs space-y-3">
 ...
 <Badge className="bg-purple-950 text-purple-300 border-purple-500/10 text-xs">{selectedRes.channel}</Badge>
 </div>
 <div className="flex items-center justify-between">
 ...
 <code className="text-muted-foreground text-xs font-mono">{selectedRes.id}</code>
 </div>
 </div>

 {/* Supervisor Release Override Trigger */}
 <div className="bg-rose-500/5 p-4 border border-border rounded-xs space-y-3">
 ...
 <p className="text-xs text-muted-foreground mt-1">Authorized supervisors can bypass Booking restrictions to release blocked slots manually.</p>
 </div>
 </div>
 
 <Button 
 onClick={() => authorizeOverride(selectedRes.id)}
 className="w-full bg-rose-950/50 hover:bg-rose-600 text-rose-200 hover:text-white border border-border rounded-none text-xs font-bold tracking-wider"
 >
 <Trash2 className="w-3.5 h-3.5 mr-1.5" /> Release Block
 </Button>
 </div>

 <Button onClick={() => setSelectedRes(null)} className="w-full bg-muted/50 border border-slate-800 text-muted-foreground hover:text-white rounded-none">
 Close Details
 </Button>
 ...
 <Card className="bg-muted/50 border border-purple-900/10 rounded-none p-5 space-y-4">
