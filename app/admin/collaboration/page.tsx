...
export default function StaffCollaborationHub() {
 ...
 <Card className="bg-muted/50 border border-purple-900/10 rounded-none shadow-xl flex-1 flex flex-col">
 ...
 className={`w-full p-3 rounded-none flex items-start gap-3 transition-all text-left border ${activeChannel === ch.id ? 'bg-purple-950/30 border-purple-500/30 shadow' : 'bg-transparent border-transparent hover:bg-muted/30'}`}
 >
 ...
 <p className="text-xs text-muted-foreground truncate mt-0.5 leading-relaxed">{ch.desc}</p>
 </div>
 </button>
 ))}
 </CardContent>
 ...
 <Card className="bg-muted/50 border border-purple-900/20 rounded-none shadow-sm flex-1 flex flex-col min-h-0">
 
 {/* Active Channel Header */}
 <div className="border-b border-border bg-slate-950/40 p-4 shrink-0 flex items-center justify-between gap-4">
 <div className="min-w-0">
 <h2 className="text-sm font-bold text-white flex items-center gap-2">
 <span className="text-lg">{currentChannelInfo?.icon}</span> {currentChannelInfo?.name}
 </h2>
 <p className="text-xs text-muted-foreground truncate">{currentChannelInfo?.desc}</p>
 </div>
 <div className="shrink-0 flex items-center gap-1.5 bg-purple-950/50 px-2 py-1 border border-purple-500/10 text-xs text-purple-300 font-mono">
 ...
 <Badge variant="outline" className="text-xs py-0 px-1 border-purple-500/20 text-purple-400">{msg.senderRole}</Badge>
 <span className="text-muted-foreground font-sans">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
 </div>

 {/* Chat text box */}
 <div className={`p-3 rounded-xs border text-xs leading-relaxed ${isOwn ? (isHigh ? 'bg-amber-950/20 border-border text-slate-100 shadow-md shadow-amber-950/10' : 'bg-purple-950/20 border-purple-500/20 text-slate-100') : (isHigh ? 'bg-rose-950/20 border-border text-slate-100 shadow-md shadow-rose-950/10' : 'bg-muted/30 border-slate-800 text-slate-300')} relative`}>
 {msg.isPending && (
 <Badge className="absolute -top-2.5 -right-2 bg-amber-950 text-amber-400 border border-border text-xs font-mono py-0 px-1 font-bold">Pending Sync</Badge>
 )}
 <p>{msg.text}</p>
 </div>
 ...
 {isHigh && (
 <div className="flex items-center gap-1.5 mt-1 text-xs tracking-wider font-bold text-amber-500 font-mono animate-pulse">
 ...
 <div className="px-4 py-1.5 bg-slate-950/40 border-t border-border text-xs text-muted-foreground italic flex items-center gap-2">
 ...
 <form onSubmit={handleSendMessage} className="p-4 border-t border-border bg-slate-950/30 shrink-0 space-y-3">
 
 <div className="flex items-center justify-between gap-4 text-xs font-mono text-muted-foreground">
 <div className="flex items-center gap-2">
 <span className="text-xs ">Tag Priority:</span>
 <button
 type="button"
 onClick={() => setMessagePriority('low')}
 className={`px-2 py-0.5 border rounded-xs text-xs ${messagePriority === 'low' ? 'bg-blue-950/30 border-blue-500/40 text-blue-400 font-bold' : 'bg-slate-950 border-slate-800 hover:border-slate-700 text-muted-foreground'}`}
 >
 Low
 </button>
 <button
 ...
 className={`px-2 py-0.5 border rounded-xs text-xs ${messagePriority === 'normal' ? 'bg-slate-950 border-slate-800 text-muted-foreground font-bold' : 'bg-slate-950 border-slate-800 hover:border-slate-700 text-muted-foreground'}`}
 >
 Normal
 </button>
 <button
 ...
 className={`px-2 py-0.5 border rounded-xs text-xs ${messagePriority === 'high' ? 'bg-amber-950/30 border-border text-amber-400 font-bold' : 'bg-slate-950 border-slate-800 hover:border-slate-700 text-muted-foreground'}`}
 >
 High SLA
 </button>
 </div>
 {!isConnected && (
 <span className="text-xs text-amber-500 flex items-center gap-1 font-bold animate-pulse"><AlertTriangle className="w-3.5 h-3.5" /> Queue Buffered</span>
 )}
 </div>
 ...
 <div>
 <CardTitle className="text-xs font-bold text-slate-300">Staff Presence</CardTitle>
 <CardDescription className="text-xs text-muted-foreground">Connected operators on active shifts.</CardDescription>
 </div>
 <Badge className="bg-emerald-950 text-emerald-300 border-border text-xs">{onlineStaff.length} Connected</Badge>
 </CardHeader>
 <CardContent className="p-3 flex-1 overflow-y-auto space-y-3">
 {onlineStaff.length === 0 ? (
 <div className="text-muted-foreground text-center py-8 text-xs">No online staff detected.</div>
 ) : (
 onlineStaff.map(staff => (
 <div key={staff.id} className="flex items-center justify-between gap-3 bg-slate-950/20 p-2.5 border border-border">
 ...
 <span className="text-xs text-muted-foreground truncate block font-mono">{staff.email}</span>
 </div>

 </div>

 <Badge variant="outline" className="text-xs border-border text-emerald-400 font-mono shrink-0 py-0.5 px-1.5">{staff.role}</Badge>
 </div>
 ))
 )}
 </CardContent>
 ...
 <Card className="bg-[#10072d]/20 border border-border rounded-none p-4">
 <div className="flex items-center gap-2 text-purple-400 mb-1.5">
 <ShieldCheck className="w-4 h-4 text-purple-400" />
 <h4 className="font-bold text-xs text-slate-300">Staff SLA Governance</h4>
 </div>
