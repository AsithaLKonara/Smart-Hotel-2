...
const STATUS_STYLE: Record<string, string> = {
 APPROVED: 'bg-emerald-500/20 text-emerald-400 border-border',
 REJECTED: 'bg-rose-500/20 text-rose-400 border-border',
 PENDING: 'bg-amber-500/20 text-amber-400 border-border',
}
...
export default function LeavesPage() {
 ...
 {leaves.map(leave => (
 <div key={leave.id} className="bg-muted/50 border border-border rounded-lg p-6 hover:border-white/20 transition-all shadow-xl">
 ...
 <p className="text-xs text-white/50 tracking-wider mt-0.5">{leave.type} LEAVE</p>
 </div>
 </div>
 <Badge className={`text-xs font-bold border ${STATUS_STYLE[leave.status] || STATUS_STYLE.PENDING}`}>
 {leave.status}
 </Badge>
 </div>

 <div className="bg-[#1a1a24] rounded-xl p-4 border border-border mb-4 text-center">
 ...
 className="flex-1 border-border text-rose-400 hover:bg-rose-500/10 text-xs"
 >
 ...
 {leaves.length === 0 && (
 <p className="text-muted-foreground col-span-3 text-center py-12">No leave requests found.</p>
 )}
 </div>
 )}
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Leave Type</label>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Start Date</label>
 <Input type="date" required value={formData.startDate} onChange={e => setFormData({ ...formData, startDate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">End Date</label>
 <Input type="date" required value={formData.endDate} onChange={e => setFormData({ ...formData, endDate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Reason (optional)</label>
 <textarea value={formData.reason} onChange={e => setFormData({ ...formData, reason: e.target.value })}
 className="w-full bg-[#1a1a24] border border-border rounded-lg p-3 text-sm text-white h-24 resize-none focus:outline-none focus:ring-1 focus:ring-primary" />
 </div>
