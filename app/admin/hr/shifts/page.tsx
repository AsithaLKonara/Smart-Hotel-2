...
export default function ShiftsPage() {
 ...
 className="border-border text-rose-400 bg-rose-500/10 hover:bg-rose-500/20">
 ...
 <tr>
 <th className="p-4 text-white/50 text-xs ">Employee</th>
 <th className="p-4 text-white/50 text-xs ">Date</th>
 <th className="p-4 text-white/50 text-xs ">Time</th>
 <th className="p-4 text-white/50 text-xs ">Role</th>
 ...
 <div className="flex items-center gap-2"><CalendarIcon className="w-4 h-4 text-muted-foreground" />
 ...
 <td className="p-4">
 <span className={`px-2 py-1 rounded text-xs font-bold ${shift.status === 'COMPLETED' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-muted-foreground'}`}>
 {shift.status}
 </span>
 </td>
 </tr>
 ...
 {shifts.length === 0 && (
 <tr><td colSpan={5} className="p-12 text-center text-muted-foreground">No shifts scheduled.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Employee</label>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Start Time</label>
 <Input type="datetime-local" required value={formData.startTime} onChange={e => setFormData({ ...formData, startTime: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">End Time</label>
 <Input type="datetime-local" required value={formData.endTime} onChange={e => setFormData({ ...formData, endTime: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Role Override (Optional)</label>
 <Input value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} placeholder="e.g. Night Manager" className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Notes</label>
 <Input value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 ...
 <div className="flex items-start gap-3 p-4 bg-rose-500/10 border border-border rounded-xl mb-5">
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Attendance Record ID</label>
