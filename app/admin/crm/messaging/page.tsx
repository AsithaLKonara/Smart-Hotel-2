...
export default function MessagingCenter() {
 ...
 <p className="text-muted-foreground">Communicate with guests and configure AI Concierge</p>
 </div>
 </div>
 ...
 className={`p-4 rounded-xl cursor-pointer border transition-all ${activeConvId === conv.id ? 'bg-[#2a2a2a] border-white/20' : 'bg-[#1a1a1a] border-transparent hover:bg-muted/50'}`}
 >
 ...
 <Card className="flex-1 bg-[#1a1a1a] border-border text-white flex flex-col overflow-hidden">
 {activeConv ? (
 <>
 {/* Chat Header */}
 ...
 <div className="text-xs text-muted-foreground">{activeConv.guest?.email}</div>
 </div>
 </div>
 <div>
 <Button 
 ...
 className={activeConv.aiEnabled ? 'border-border text-emerald-400' : 'border-slate-500/50 text-muted-foreground'}
 >
 ...
 <div className={`text-xs mt-1 text-right flex items-center justify-end gap-1 ${isGuest ? 'text-muted-foreground' : 'text-emerald-200'}`}>
 ...
 className="flex-1 p-3 rounded-xl bg-black/50 border border-border focus:outline-none focus:border-white/30"
 />
