...
export default function GuestProfileDashboard() {
 ...
 <Button variant="ghost" className="-ml-4 text-muted-foreground hover:text-slate-900" onClick={() => router.back()}>
 ...
 <span className="flex items-center"><Mail className="w-4 h-4 mr-2 text-muted-foreground" /> {guest.email}</span>
 {guest.phone && <span className="flex items-center"><Phone className="w-4 h-4 mr-2 text-muted-foreground" /> {guest.phone}</span>}
 </div>
 </div>
 
 {guest.loyalty && (
 <div className="bg-slate-50 p-4 rounded-lg border text-center min-w-[200px]">
 <div className="text-xs font-semibold text-muted-foreground tracking-wider mb-1">Loyalty Tier</div>
 ...
 <div className="text-sm font-mono mt-1 text-muted-foreground">{guest.loyalty.points.toLocaleString()} Points</div>
 </div>
 )}
 </div>
 ...
 <div className="flex justify-between items-center">
 <span className="text-muted-foreground text-sm">Total Stays</span>
 ...
 <div>
 <h4 className="text-xs font-semibold text-muted-foreground mb-2">Dietary</h4>
 <div className="flex flex-wrap gap-2">
 ...
 <div>
 <h4 className="text-xs font-semibold text-muted-foreground mb-2">Room Setup</h4>
 <ul className="text-sm space-y-1 text-slate-700">
 {guest.guestPreferences.roomPreferences.map((r: string) => (
 <li key={r} className="flex items-center">
 <Bed className="w-3 h-3 mr-2 text-muted-foreground" /> {r}
 </li>
 ))}
 </ul>
 </div>
 ...
 <div className="text-muted-foreground text-sm text-center py-4">No recent bookings found.</div>
 ) : (
 ...
 booking.status === 'COMPLETED' ? 'bg-slate-100 text-muted-foreground' : ''
 }>
 {booking.status}
 </Badge>
