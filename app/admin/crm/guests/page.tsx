...
export default function GuestCRMPage() {
 ...
 className={vipOnly ? 'bg-primary text-white border-primary' : 'bg-muted/50 border-border text-white'}
 >
 ...
 <div className="bg-muted/50 border border-border rounded-xl overflow-hidden shadow-sm">
 ...
 {guests.length === 0 && (
 <tr><td colSpan={5} className="p-12 text-center text-muted-foreground">No guests found matching your criteria.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 <p className="text-muted-foreground">{selectedGuest.email} {selectedGuest.phone ? `• ${selectedGuest.phone}` : ''}</p>
 </div>
 ...
 <div>
 <label className="text-xs text-white/60 mb-2 block tracking-wider font-bold">Loyalty Tier</label>
 <select 
 className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm focus:ring-1 focus:ring-primary outline-none"
 value={editVip}
 ...
 <div>
 <label className="text-xs text-white/60 mb-2 block tracking-wider font-bold flex items-center">
 <ShieldAlert className="w-3 h-3 mr-1 text-rose-500" /> Account Restriction
 </label>
 <label className="flex items-center gap-2 text-sm p-2 bg-[#1a1a24] rounded-lg border border-border cursor-pointer">
 <input 
 type="checkbox" 
 ...
 className="bg-[#1a1a24] border-border text-white placeholder:text-muted-foreground"
 />
 </div>
 )}
