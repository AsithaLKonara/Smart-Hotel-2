...
export default function YieldManagementPage() {
 ...
 <Input value={newRule.name} onChange={e => setNewRule({...newRule, name: e.target.value})} className="bg-muted/50 border-border" placeholder="e.g. 80%+ Compression Surge" />
 </div>
 <div className="flex gap-4">
 <div className="flex-1">
 <label className="text-xs text-white/60 mb-1 block">Min Occupancy (0.0-1.0)</label>
 <Input type="number" step="0.05" value={newRule.minOccupancy} onChange={e => setNewRule({...newRule, minOccupancy: parseFloat(e.target.value)})} className="bg-muted/50 border-border" />
 </div>
 <div className="flex-1">
 <label className="text-xs text-white/60 mb-1 block">Max Occupancy (0.0-1.0)</label>
 <Input type="number" step="0.05" value={newRule.maxOccupancy} onChange={e => setNewRule({...newRule, maxOccupancy: parseFloat(e.target.value)})} className="bg-muted/50 border-border" />
 </div>
 </div>
 <div>
 <label className="text-xs text-white/60 mb-1 block">Adjustment Percentage (%)</label>
 <Input type="number" value={newRule.adjustmentValue} onChange={e => setNewRule({...newRule, adjustmentValue: parseFloat(e.target.value)})} className="bg-muted/50 border-border" placeholder="e.g. 15 for +15%" />
 </div>
 <div>
 <label className="text-xs text-white/60 mb-1 block">Competitor Parity Cap ($)</label>
 <Input type="number" value={newRule.competitorPrice} onChange={e => setNewRule({...newRule, competitorPrice: e.target.value})} className="bg-muted/50 border-border" placeholder="e.g. 250 (Optional)" />
 </div>
 ...
 {rules.map((rule: any) => (
 <Card key={rule.id} className={`bg-[#1a1a1a] border-border ${!rule.isActive ? 'opacity-50' : ''}`}>
 ...
 <p className="text-xs text-muted-foreground tracking-wider mb-1">Adjustment</p>
 ...
 <div>
 <Card className="bg-[#1a1a1a] border-border sticky top-6">
