...
export default function UnifiedLiveTimeline() {
 ...
 return (
 <div className="min-h-screen bg-background text-slate-100 p-6 font-sans relative">
 ...
 <p className="text-muted-foreground text-sm mt-0.5">Real-time cross-department workflow dispatch center. Orchestrating staff actions, guests arrival/checkout blocks, and SLA incident timers.</p>
 </div>
 <div className="flex items-center gap-3">
 <Button onClick={triggerManualSimulationEvent} variant="outline" size="sm" className="bg-muted/50 border-border text-amber-300">
 <Zap className="w-4 h-4 mr-1.5 text-amber-400 animate-pulse" /> Inject Test Event
 </Button>
 <Button onClick={() => router.push('/admin/collaboration')} variant="outline" size="sm" className="bg-muted/50 border-border text-purple-300">
 Team Message Desk
 </Button>
 ...
 <div className="bg-muted/30 border border-border p-4 rounded-none">
 <span className="text-xs font-bold text-muted-foreground block">Total Active Incidents</span>
 <strong className="text-2xl text-white font-serif mt-1 block">{events.length}</strong>
 </div>
 <div className="bg-muted/30 border border-border p-4 rounded-none">
 <span className="text-xs font-bold text-muted-foreground block">Critical Breaches</span>
 <strong className="text-2xl text-rose-500 font-serif mt-1 block">{events.filter(e => e.severity === 'critical').length}</strong>
 </div>
 <div className="bg-muted/30 border border-border p-4 rounded-none">
 <span className="text-xs font-bold text-muted-foreground block">SLA Warnings</span>
 <strong className="text-2xl text-amber-500 font-serif mt-1 block">{events.filter(e => e.severity === 'warning').length}</strong>
 </div>
 <div className="bg-muted/30 border border-border p-4 rounded-none">
 <span className="text-xs font-bold text-muted-foreground block">Assigned Staff Nodes</span>
 <strong className="text-2xl text-purple-400 font-serif mt-1 block">{onlineStaffCountPlusDefault()} Operators</strong>
 </div>
 <div className="bg-muted/30 border border-border p-4 rounded-none col-span-2 md:col-span-1">
 <span className="text-xs font-bold text-muted-foreground block">System Connection Status</span>
 ...
 <Card className="bg-muted/50 border border-purple-900/10 rounded-none shadow-sm relative overflow-hidden">
 
 <div className="p-4 border-b border-border bg-slate-950/40 flex flex-col md:flex-row items-center justify-between gap-4">
 <div className="flex items-center gap-2">
 <SlidersHorizontal className="w-4 h-4 text-purple-400 animate-pulse" />
 <h3 className="text-xs font-bold text-white">Hourly Gantt-Style Dispatch Timeline</h3>
 </div>
 <div className="flex items-center gap-1">
 <span className="text-xs text-muted-foreground font-bold mr-2">Filter Department:</span>
 {['ALL', 'RECEPTION', 'HOUSEKEEPING', 'MAINTENANCE', 'SECURITY'].map(cat => (
 <button
 key={cat}
 onClick={() => setFilterCategory(cat)}
 className={`px-2.5 py-1 border rounded-xs text-xs transition-colors ${filterCategory === cat ? 'bg-purple-950/30 border-purple-500/40 text-purple-300 font-bold' : 'bg-transparent border-transparent hover:bg-muted/30 text-muted-foreground'}`}
 >
 {cat === 'ALL' ? 'All' : cat}
 </button>
 ))}
 ...
 <div className="grid grid-cols-9 border-b border-border pb-3 text-center text-xs text-muted-foreground font-bold tracking-wider">
 <div className="text-left font-sans text-muted-foreground text-xs">Department Track</div>
 {HOURS.map(hr => (
 <div key={hr} className="border-l border-border flex items-center justify-center gap-1"><Clock className="w-3 h-3 text-muted-foreground" /> {hr}</div>
 ))}
 </div>
 ...
 return (
 <div key={category} className="grid grid-cols-9 py-6 relative items-center group hover:bg-muted/30">
 
 {/* Title Track column */}
 <div className="text-left font-bold text-xs tracking-wider text-muted-foreground flex items-center gap-1.5">
 ...
 <div className="col-span-8 h-8 relative w-full bg-slate-950/10 border border-border rounded-xs">
 {rowEvents.map(event => {
 ...
 if (event.severity === 'warning') colorClass = "bg-amber-950/30 border-border text-amber-300"
 if (event.severity === 'critical') colorClass = "bg-rose-950/30 border-border text-rose-300 shadow shadow-rose-950"
 ...
 className={`absolute top-[10%] h-[80%] border rounded-xs px-2.5 flex items-center justify-between text-xs truncate transition-all hover:scale-[1.02] hover:z-20 cursor-pointer ${colorClass}`}
 >
 <span className="font-bold truncate mr-2">{event.title}</span>
 <Badge className="bg-slate-950/50 hover:bg-slate-950/50 text-xs py-0 px-1 shrink-0 font-mono text-muted-foreground">{event.time}</Badge>
 </button>
 )
 })}
 </div>
 ...
 {selectedEvent && (
 <Card className="bg-muted/50 border border-purple-500/40 rounded-none shadow-sm relative">
 <CardHeader className="bg-purple-950/30 border-b border-border p-4">
 <div className="flex items-center justify-between">
 <Badge variant="outline" className={`text-xs font-bold py-0.5 px-2 ${selectedEvent.severity === 'critical' ? 'bg-rose-950 text-rose-400 border-border' : selectedEvent.severity === 'warning' ? 'bg-amber-950 text-amber-400 border-border' : 'bg-purple-950 text-purple-400 border-purple-500/20'}`}>
 {selectedEvent.severity}
 </Badge>
 <button onClick={() => setSelectedEvent(null)} className="text-muted-foreground hover:text-white text-xs">Dismiss Inspector</button>
 </div>
 <CardTitle className="text-sm font-serif font-bold text-white mt-2">{selectedEvent.title}</CardTitle>
 <CardDescription className="text-xs text-muted-foreground flex items-center gap-1.5"><Clock className="w-3 h-3 text-muted-foreground" /> Target dispatch window: {selectedEvent.time}</CardDescription>
 </CardHeader>
 <CardContent className="p-4 space-y-4 text-xs">
 
 <div className="p-3 bg-slate-950 border border-border rounded-xs space-y-1.5">
 <span className="text-xs text-muted-foreground font-bold font-mono block">PROCESS DISPATCH MESSAGE</span>
 <p className="text-slate-300 leading-relaxed text-[11px]">{selectedEvent.message}</p>
 </div>

 <div className="grid grid-cols-2 gap-3 text-xs">
 <div className="bg-slate-950/30 p-2 border border-border">
 <span className="text-muted-foreground font-mono block text-xs font-bold">Assigned Staff</span>
 <strong className="text-slate-300 truncate block mt-0.5">{selectedEvent.assignedStaff || 'Unassigned'}</strong>
 </div>
 <div className="bg-slate-950/30 p-2 border border-border">
 <span className="text-muted-foreground font-mono block text-xs font-bold">Target Room</span>
 ...
 <Card className="bg-[#0b061c]/60 border border-border rounded-none shadow-xl flex flex-col">
 <CardHeader className="border-b border-border p-4 shrink-0 flex flex-row items-center justify-between">
 <div>
 ...
 <CardDescription className="text-xs text-muted-foreground">Live system events as they occur across rooms.</CardDescription>
 </div>
 ...
 <CardContent className="p-3 max-h-[300px] overflow-y-auto space-y-2 font-mono text-xs">
 
 {logsFeed.map(log => (
 <div key={log.id} className="p-2 border border-border bg-slate-950/30 flex items-start gap-2.5 leading-relaxed">
