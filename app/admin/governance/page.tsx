...
export default function GovernancePage() {
 ...
 <span className="text-xs tracking-[0.25em] text-[#D4AF37] font-medium">Compliance & Regulatory Control</span>
 ...
 {certs.map((cert) => (
 <div key={cert.id} className="p-6 rounded-lg bg-[#121118]/85 border border-white/[0.04] flex flex-col justify-between min-h-[220px]">
 <div className="space-y-3">
 <div className="flex justify-between items-center">
 <span className="text-xs font-mono px-2 py-0.5 rounded bg-white/[0.03] text-[#8E8C94] border border-white/[0.04]">
 {cert.standard}
 </span>
 <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${
 cert.status === 'CERTIFIED' ? 'bg-emerald-500/10 text-emerald-400 border border-border' : 'bg-red-500/10 text-red-400 border border-red-500/20'
 }`}>
 {cert.status}
 </span>
 </div>
 ...
 <span className="text-xs text-[#8E8C94] block">Last Compiled: {cert.lastEvaluated}</span>
 <span className="text-xs text-[#D4AF37] font-mono block">SHA-256: {cert.integritySignature}</span>
 </div>
 <button
 onClick={() => handleCompileReport(cert.id)}
 disabled={compilingId !== null}
 className="px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider bg-white/[0.03] text-white border border-border hover:bg-white/[0.08] transition-all disabled:opacity-40"
 >
 ...
 <div className="lg:col-span-4 bg-[#121118]/85 border border-white/[0.04] rounded-lg p-6 space-y-5">
 ...
 <p className="text-xs text-[#8E8C94]">All balance transactions match hashes.</p>
 </div>
 </label>
 ...
 <p className="text-xs text-[#8E8C94]">Simulation verified multi-region replica redundancy.</p>
 </div>
 </label>
 ...
 <p className="text-xs text-[#8E8C94]">Intrusion models verified no token vulnerabilities.</p>
 </div>
 </label>
 </div>
 ...
 className="w-full py-2.5 rounded-xl text-xs font-bold tracking-wider bg-gradient-to-r from-[#D4AF37] to-[#9C8259] text-[#09050D] hover:opacity-95 transition-all disabled:opacity-40"
 >
