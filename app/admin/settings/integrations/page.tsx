...
export default function IntegrationsPage() {
 ...
 <p className="text-muted-foreground">Connect SmartHotel OS to external systems via Apps and Webhooks.</p>
 </div>
 ...
 <Card key={app.id} className="bg-[#1a1a1a] border-border text-white">
 <CardContent className="p-4 flex items-center justify-between">
 <div className="flex items-center gap-4">
 <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center border border-border">
 <LinkIcon className="w-6 h-6 text-muted-foreground" />
 </div>
 <div>
 <h3 className="font-bold">{app.appName}</h3>
 <p className="text-xs text-muted-foreground tracking-wider">{app.provider}</p>
 </div>
 </div>
 ...
 className={app.status === 'ACTIVE' ? 'border-red-500/50 text-red-400 hover:bg-red-500/10' : 'border-border text-emerald-400 hover:bg-emerald-500/10'}
 >
 ...
 <Card className="bg-[#1a1a1a] border-border text-white mb-6">
 <CardContent className="p-4">
 <form onSubmit={handleAddWebhook} className="flex gap-2">
 <select 
 className="bg-black border border-border rounded-md p-2 text-sm focus:outline-none focus:border-white/30"
 ...
 className="flex-1 bg-black border border-border rounded-md p-2 text-sm focus:outline-none focus:border-white/30"
 ...
 {webhooks.map(hook => (
 <Card key={hook.id} className="bg-black/50 border-border border-l-4 border-l-purple-500 text-white">
 <CardContent className="p-4">
 ...
 Secret: <span className="font-mono text-muted-foreground blur-[2px] hover:blur-none transition-all cursor-pointer">{hook.secret}</span>
 </div>
 </CardContent>
 </Card>
 ))}
