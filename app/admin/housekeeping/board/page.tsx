...
export default function MobileHousekeepingBoard() {
 ...
 <button onClick={() => queryClient.invalidateQueries({ queryKey: ['attendant-tasks'] })} className="p-3 bg-muted/50 rounded-full text-white hover:bg-white/10 active:scale-95 transition-all">
 ...
 <h3 className="text-3xl font-bold text-white">Room {task.room?.number || '??'}</h3>
 ...
 className="p-4 bg-rose-500/10 border border-border rounded-xl text-rose-500 active:scale-95 transition-all flex items-center justify-center"
 >
 ...
 <div className="p-8 text-center bg-muted/50 border border-border rounded-lg">
 <p className="text-muted-foreground text-sm">No pending tasks.</p>
 </div>
 ) : (
 <div className="space-y-4">
 {pendingTasks.map((task: any) => (
 <Card key={task.id} className="bg-[#111] border-border shadow-sm">
 ...
 <h2 className="text-xs font-bold text-amber-500 mb-3 flex items-center gap-2">
 <AlertTriangle className="w-4 h-4" /> Awaiting Inspection ({completedTasks.length})
 </h2>
 <div className="space-y-3">
 {completedTasks.map((task: any) => (
 <div key={task.id} className="p-4 bg-muted/50 border border-border rounded-xl flex justify-between items-center">
