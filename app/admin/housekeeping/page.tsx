...
export default function HousekeepingOperations() {
 ...
 <p className="text-muted-foreground text-sm mt-1">Dispatch cleaning tasks and inspect rooms.</p>
 </div>
 <Button variant="outline" onClick={() => {
 queryClient.invalidateQueries({ queryKey: ['housekeeping-tasks'] })
 queryClient.invalidateQueries({ queryKey: ['housekeeping-rooms'] })
 }} className="bg-muted/50 border-border mt-4 md:mt-0 text-white">
 ...
 className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${activeTab === tab ? 'bg-primary text-white shadow-md' : 'text-muted-foreground hover:text-white'}`}
 >
 {tab.replace('_', ' ')}
 </button>
 ))}
 ...
 <div key={room.id} className="p-4 bg-white/[0.03] border border-border rounded-lg flex flex-col items-center justify-center text-center">
 ...
 {roomsData?.length === 0 && <p className="col-span-full text-center text-muted-foreground p-8">No rooms found.</p>}
 </div>
 ...
 <p className="text-muted-foreground font-medium">No active tasks in this view.</p>
 </div>
 ) : (
 ...
 className="p-5 bg-white/[0.03] border border-border rounded-lg hover:border-white/20 transition-all shadow-xl"
 >
 ...
 <Badge className={task.status === 'IN_PROGRESS' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-500/20 text-muted-foreground'}>
 {task.status.replace('_', ' ')}
 </Badge>
 </div>

 <div className="mb-4">
 <label className="text-xs font-bold text-muted-foreground tracking-wider mb-1 block">Assigned Attendant</label>
 <select 
 className="w-full bg-[#0a0a0f] border border-border rounded-lg p-2 text-sm text-white focus:ring-1 focus:ring-primary outline-none"
 ...
 <Card className="bg-[#0a0a0f] border-border shadow-sm">
 <CardContent className="p-0">
 {inspectionTasks.length === 0 ? (
 <div className="text-center py-16 px-4">
 <CheckCircle2 className="w-12 h-12 text-emerald-500/20 mx-auto mb-3" />
 <p className="text-emerald-500/50 text-xs font-bold ">All Rooms Inspected</p>
 </div>
 ) : (
 <div className="divide-y divide-white/5">
 {inspectionTasks.map((task: any) => (
 <div key={task.id} className="p-5 hover:bg-muted/50 transition-colors">
 <div className="flex justify-between items-start mb-3">
 <div>
 ...
 <Badge className="bg-amber-500/10 text-amber-500 border-border">Pending</Badge>
 </div>
 ...
 variant="outline" 
 className="w-12 border-border hover:bg-rose-500/10 text-rose-500"
