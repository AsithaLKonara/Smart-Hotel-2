...
export default function GlobalCommandCenter() {
 ...
 <span className="text-xs tracking-[0.25em] text-[#D4AF37] font-medium">Worldwide Operational Watch</span>
 ...
 <div className="bg-[#121118]/85 border border-white/[0.04] rounded-lg p-5">
 <span className="text-xs text-[#8E8C94] tracking-wider block">Platform SLA Availability</span>
 ...
 <div className="bg-[#121118]/85 border border-white/[0.04] rounded-lg p-5">
 <span className="text-xs text-[#8E8C94] tracking-wider block">Edge Error Rate</span>
 ...
 <span className="text-xs text-emerald-400/80 font-mono mt-2 block">Last 60 Minutes</span>
 </div>

 <div className="bg-[#121118]/85 border border-white/[0.04] rounded-lg p-5">
 <span className="text-xs text-[#8E8C94] tracking-wider block">API Median Latency</span>
 <span className="text-2xl font-light font-serif text-white mt-2 block">{healthData?.api?.latency}ms P95</span>
 <span className="text-xs text-[#8E8C94] font-mono mt-2 block">Routing within limits</span>
 </div>

 <div className="bg-[#121118]/85 border border-white/[0.04] rounded-lg p-5">
 <span className="text-xs text-[#8E8C94] tracking-wider block">Outbox Queue Depth</span>
 ...
 <span className="text-xs text-emerald-400/80 font-mono mt-2 block">Max latency: {healthData?.queue?.latencySeconds}s</span>
 </div>
 </div>
 ...
 <div className="lg:col-span-8 bg-[#121118]/85 border border-white/[0.04] rounded-lg p-6 space-y-4">
 <h2 className="text-lg font-serif font-light text-white tracking-wider">Infrastructure Node Diagnostics</h2>
 
 <div className="overflow-x-auto">
 <table className="w-full text-left text-sm border-collapse">
 <thead>
 <tr className="border-b border-white/[0.04] text-[#8E8C94] text-xs tracking-wider">
 ...
 <div className="text-xs text-[#8E8C94] font-mono mt-0.5">{node.lat} • {node.lon}</div>
 </td>
 <td className="py-4">
 <span className={`px-2 py-0.5 rounded text-xs font-bold ${
 node.status === 'HEALTHY' ? 'bg-emerald-500/10 text-emerald-400 border border-border' :
 node.status === 'WARNING' || node.status === 'DEGRADED' ? 'bg-amber-500/10 text-amber-400 border border-border' :
 ...
 <div className="lg:col-span-4 bg-black/40 border border-white/[0.02] rounded-lg p-6 min-h-[350px] flex flex-col justify-between">
 <div className="space-y-4">
 <span className="text-xs tracking-wider text-[#8E8C94] font-medium block border-b border-white/[0.04] pb-2 flex justify-between">
 SRE Operations Telemetry Stream
 <span className="text-emerald-400 font-bold animate-pulse text-xs">LIVE</span>
 </span>
 ...
 <div key={idx} className={`p-2 border rounded ${log.severity === 'ERROR' ? 'border-border bg-rose-500/5 text-rose-300' : 'border-border bg-muted/50'}`}>
 <div className="text-white mb-1 font-sans font-semibold text-xs flex items-center justify-between">
 {log.type} <span className="text-[#8E8C94] font-mono text-xs font-normal">{log.time}</span>
 </div>
 {log.message}
 </div>
 ))}
