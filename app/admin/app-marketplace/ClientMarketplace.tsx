...
export interface MarketplaceApp {
 id: string;
 title: string;
 category: 'INTEGRATION' | 'AI_MODULE' | 'FINANCIAL' | 'MARKETING' | string;
 description: string;
 rating: number;
 installed: boolean;
 cost: string;
...
export interface PartnerVendor {
 ...
 status: 'ACTIVE' | 'SUSPENDED' | 'REVIEW' | string;
 lastPayout: number;
}

export default function ClientMarketplace({ initialApps, initialVendors }: { initialApps: MarketplaceApp[], initialVendors: PartnerVendor[] }) {
 const [apps, setApps] = useState<MarketplaceApp[]>(initialApps);
 const [vendors, setVendors] = useState<PartnerVendor[]>(initialVendors);
 ...
 <span className="text-xs tracking-[0.25em] text-[#D4AF37] font-medium">SmartHotel Ecosystem</span>
 ...
 <div
 key={app.id}
 className={`p-6 rounded-lg border transition-all duration-300 bg-[#121118]/85 flex flex-col justify-between min-h-[220px] ${
 app.installed 
 ...
 <div>
 <span className="text-xs font-mono font-bold px-2 py-0.5 rounded bg-white/[0.03] text-[#D4AF37] border border-white/[0.04]">
 {app.category}
 </span>
 ...
 : 'bg-white/[0.03] text-white border border-border hover:bg-white/[0.08]'
 }`}
 >
 ...
 vend.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-400 border border-border' :
