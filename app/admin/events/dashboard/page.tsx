...
function ModalShell({ open, onClose, title, children }: { open: boolean, onClose: () => void, title: string, children: React.ReactNode }) {
 ...
 <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-[#0a0a0f] border border-border rounded-lg shadow-sm p-6 z-50 max-h-[90vh] overflow-y-auto">
...
function FormField({ label, children }: { label: string, children: React.ReactNode }) {
 return (
 <div>
 <label className="text-xs font-bold text-white/60 tracking-wider mb-2 block">{label}</label>
 {children}
 </div>
 )
}
...
export default function EventsDashboard() {
 ...
 className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab ? 'bg-primary text-white shadow-md' : 'text-muted-foreground hover:text-white'}`}
 >
 {tab}
 </button>
 ))}
 ...
 <Button variant="outline" onClick={() => setQuoteModal(true)} className="bg-muted/50 border-border text-white hover:bg-white/10">
 ...
 <Card key={event.id} className="bg-muted/50 border-border hover:border-white/20 transition-all shadow-xl">
 <CardContent className="p-6 flex flex-col gap-4">
 <div className="flex justify-between items-start">
 <div>
 <h3 className="font-bold text-white text-lg">{event.name}</h3>
 <p className="text-xs text-white/50 tracking-wider mt-1">{event.type}</p>
 </div>
 <Badge className={`text-xs px-2 py-1 rounded-full border ${
 event.status === 'DEFINITE' ? 'bg-emerald-500/20 text-emerald-400 border-border' :
 event.status === 'TENTATIVE' ? 'bg-amber-500/20 text-amber-400 border-border' :
 ...
 <div className="flex flex-col gap-2 text-sm text-white/60 bg-[#1a1a24] p-4 rounded-xl border border-border">
 ...
 <div>
 <p className="text-xs text-muted-foreground mb-2 tracking-wider font-bold">Group Blocks</p>
 {event.groupBlocks.map((block: any) => (
 <div key={block.id} className="flex justify-between items-center bg-muted/50 p-3 rounded-lg text-sm mb-2 border border-border">
 ...
 <Card key={space.id} className="bg-muted/50 border-border hover:border-white/20 transition-all shadow-xl">
 <CardContent className="p-6">
 <div className="flex justify-between items-start mb-4">
 <h3 className="font-bold text-xl text-white">{space.name}</h3>
 <Badge className={`text-xs ${space.isActive ? 'bg-emerald-500/20 text-emerald-400 border-border' : 'bg-slate-500/20 text-muted-foreground border-slate-500/30'}`}>
 {space.isActive ? 'Active' : 'Inactive'}
 </Badge>
 </div>
 <div className="grid grid-cols-3 gap-3 text-center">
 <div className="bg-[#1a1a24] p-3 rounded-xl border border-border">
 ...
 <tr>
 <th className="p-4 text-white/50 text-xs ">Event</th>
 <th className="p-4 text-white/50 text-xs ">Room Type</th>
 <th className="p-4 text-white/50 text-xs ">Blocked</th>
 <th className="p-4 text-white/50 text-xs ">Picked Up</th>
 <th className="p-4 text-white/50 text-xs ">Rate</th>
 <th className="p-4 text-right text-white/50 text-xs ">Actions</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-white/10">
 {blocks.map((block: any) => (
 <tr key={block.id} className="hover:bg-muted/50 transition-colors">
 ...
 <td className="p-4">
 <span className={`font-bold ${block.pickedUpCount > 0 ? 'text-emerald-400' : 'text-muted-foreground'}`}>{block.pickedUpCount}</span>
 </td>
 ...
 {blocks.length === 0 && (
 <tr><td colSpan={6} className="p-12 text-center text-muted-foreground">No group blocks found.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 <select required value={quoteForm.roomTypeId} onChange={e => setQuoteForm({ ...quoteForm, roomTypeId: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <Input type="date" required value={quoteForm.checkIn} onChange={e => setQuoteForm({ ...quoteForm, checkIn: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <FormField label="Check-Out">
 <Input type="date" required value={quoteForm.checkOut} onChange={e => setQuoteForm({ ...quoteForm, checkOut: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 </div>
 <FormField label="Number of Guests">
 <Input type="number" min="1" required value={quoteForm.guests} onChange={e => setQuoteForm({ ...quoteForm, guests: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 ...
 <p className="text-xs text-primary font-bold mb-3 flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Quote Generated</p>
 ...
 <Input required placeholder="Paste the Guest's User ID" value={attendanceForm.userId} onChange={e => setAttendanceForm({ ...attendanceForm, userId: e.target.value })} className="bg-[#1a1a24] border-border text-white font-mono text-sm" />
 </FormField>
 <FormField label="Number of Attendees">
 <Input type="number" min="1" required value={attendanceForm.attendees} onChange={e => setAttendanceForm({ ...attendanceForm, attendees: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 ...
 <Input required value={spaceForm.name} onChange={e => setSpaceForm({ ...spaceForm, name: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="e.g. Grand Ballroom" />
 </FormField>
 <FormField label="Capacity (persons)">
 <Input type="number" min="1" required value={spaceForm.capacity} onChange={e => setSpaceForm({ ...spaceForm, capacity: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <div className="grid grid-cols-2 gap-4">
 <FormField label="Hourly Rate ($)">
 <Input type="number" min="0" step="0.01" required value={spaceForm.hourlyRate} onChange={e => setSpaceForm({ ...spaceForm, hourlyRate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <FormField label="Daily Rate ($)">
 <Input type="number" min="0" step="0.01" required value={spaceForm.dailyRate} onChange={e => setSpaceForm({ ...spaceForm, dailyRate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
 ...
 <select required value={blockForm.eventId} onChange={e => setBlockForm({ ...blockForm, eventId: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 <option value="">Select an event...</option>
 {events.map((ev: any) => <option key={ev.id} value={ev.id}>{ev.name}</option>)}
 </select>
 </FormField>
 <FormField label="Room Type">
 <select required value={blockForm.roomTypeId} onChange={e => setBlockForm({ ...blockForm, roomTypeId: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 <option value="">Select a room type...</option>
 ...
 <Input type="number" min="1" required value={blockForm.blockedCount} onChange={e => setBlockForm({ ...blockForm, blockedCount: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <FormField label="Contracted Rate ($)">
 <Input type="number" min="0" step="0.01" required value={blockForm.contractedRate} onChange={e => setBlockForm({ ...blockForm, contractedRate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
 ...
 <Input required value={eventForm.name} onChange={e => setEventForm({ ...eventForm, name: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="e.g. Annual Corporate Gala" />
 </FormField>
 <div className="grid grid-cols-2 gap-4">
 <FormField label="Event Type">
 <select value={eventForm.type} onChange={e => setEventForm({ ...eventForm, type: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <select value={eventForm.status} onChange={e => setEventForm({ ...eventForm, status: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <Input type="datetime-local" required value={eventForm.startDate} onChange={e => setEventForm({ ...eventForm, startDate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <FormField label="End Date">
 <Input type="datetime-local" required value={eventForm.endDate} onChange={e => setEventForm({ ...eventForm, endDate: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 </div>
 <FormField label="Expected Attendees">
 <Input type="number" min="1" required value={eventForm.expectedAttendees} onChange={e => setEventForm({ ...eventForm, expectedAttendees: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <div className="grid grid-cols-2 gap-4">
 <FormField label="Organizer Name">
 <Input required value={eventForm.organizerName} onChange={e => setEventForm({ ...eventForm, organizerName: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 <FormField label="Organizer Email">
 <Input type="email" value={eventForm.organizerEmail} onChange={e => setEventForm({ ...eventForm, organizerEmail: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </FormField>
 </div>
 <FormField label="Assign to Space (optional)">
 <select value={eventForm.spaceId} onChange={e => setEventForm({ ...eventForm, spaceId: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
