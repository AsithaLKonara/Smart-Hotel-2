...
export default function WorkflowAutomationBuilder() {
 ...
 <p className="text-muted-foreground text-sm mt-1">Design event-driven operational automation triggers, filter conditional parameters, and dispatch automated tasks across staff channels.</p>
 </div>
 <div className="flex items-center gap-3">
 <Button onClick={loadAutomationRules} variant="outline" size="sm" className="bg-muted/50 border-border text-purple-300">
 ...
 <Card className="bg-muted/50 border border-purple-900/20 rounded-none shadow-sm relative overflow-hidden">
 
 <CardHeader className="border-b border-border bg-slate-950/40 pb-4">
 ...
 <CardDescription className="text-xs text-muted-foreground">Compose event-driven execution routes graphically.</CardDescription>
 </CardHeader>
 ...
 <label className="text-xs font-bold text-muted-foreground">Recipe Descriptor Name</label>
 <input 
 type="text" 
 value={ruleName} 
 ...
 <div className="p-6 bg-slate-950 border border-border rounded-xs flex flex-col md:flex-row items-center justify-between gap-4 font-sans text-xs relative">
 ...
 <span className="text-xs font-bold text-purple-400 block mb-1">1. TRIGGER EVENT</span>
 <strong className="text-xs text-white truncate block">{selectedTrigger}</strong>
 </div>
 ...
 <span className="text-xs font-bold text-indigo-400 block mb-1">2. PARAM FILTER</span>
 <strong className="text-xs text-white truncate block">{conditionField} = {conditionValue}</strong>
 </div>

 <ArrowRight className="w-4 h-4 text-indigo-500 shrink-0 hidden md:block" />

 {/* 3. Action Node */}
 <div className="p-3 bg-emerald-950/40 border border-border rounded text-center w-full md:w-[150px]">
 <span className="text-xs font-bold text-emerald-400 block mb-1">3. TARGET ACTION</span>
 <strong className="text-xs text-white truncate block">{selectedAction}</strong>
 </div>
 ...
 {/* Select Trigger */}
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground">Trigger Event</label>
 <select 
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground">Parameter Field</label>
 <select 
 ...
 {/* Select Action */}
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground">Action Execution</label>
 <select 
 ...
 {/* Action Target Queue */}
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground">Recipient Dispatcher</label>
 <input 
 type="text" 
 value={actionTarget} 
 ...
 <Card className="bg-muted/50 border border-purple-900/10 rounded-none shadow-xl">
 <CardHeader className="border-b border-border p-5 flex flex-row items-center justify-between">
 <div>
 <CardTitle className="text-sm font-bold text-slate-300">Active Orchestrations</CardTitle>
 <CardDescription className="text-xs text-muted-foreground">Committed recipe rules managing staff dispatches.</CardDescription>
 </div>
 ...
 {rules.length === 0 ? (
 <div className="text-muted-foreground text-center py-12 text-xs">No active rule recipes found. Design one on the studio!</div>
 ) : (
 rules.map((rule) => (
 ...
 {!rule.active && <Badge variant="outline" className="text-xs text-muted-foreground border-slate-800">Inactive</Badge>}
 </div>
 
 <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground font-mono">
 <Badge variant="outline" className="text-xs text-purple-400 border-purple-500/20">{rule.trigger}</Badge>
 <ArrowRight className="w-3 h-3" />
 <Badge variant="outline" className="text-xs text-emerald-400 border-border">{rule.action}</Badge>
 </div>

 <div className="flex items-center gap-3 text-xs text-muted-foreground">
 <span className="flex items-center gap-1"><Play className="w-3 h-3 text-muted-foreground" /> Runs: {rule.runCount}</span>
 <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-muted-foreground" /> Created {new Date(rule.createdAt).toLocaleDateString()}</span>
 </div>
 </div>
 ...
 className="p-1.5 bg-slate-950 hover:bg-rose-950 border border-slate-800 hover:border-border text-muted-foreground hover:text-rose-400 transition-all rounded"
 >
