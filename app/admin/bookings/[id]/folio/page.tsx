...
export default function AdvancedFolioPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <p className="text-muted-foreground text-sm mt-1">Manage guest folios, corporate routing, and line items.</p>
 </div>
 <div className="flex gap-2 mt-4 md:mt-0">
 <Button variant="outline" className="bg-muted/50 border-border text-white hover:bg-white/10" 
 ...
 return (
 <Card key={folio.id} className="min-w-[400px] max-w-[450px] shrink-0 bg-[#0a0a0f] border-border snap-center flex flex-col">
 ...
 {folio.company ? (
 <p className="text-xs text-muted-foreground flex items-center"><Building2 className="w-3 h-3 mr-1" /> {folio.company.name}</p>
 ) : (
 <p className="text-xs text-muted-foreground flex items-center"><FileText className="w-3 h-3 mr-1" /> Guest Master</p>
 )}
 </div>
 <div className="text-right">
 <span className="text-xl font-bold text-emerald-400">${totalAmount.toFixed(2)}</span>
 <div className="flex items-center justify-end gap-2 mt-1">
 <span className="text-xs text-muted-foreground block tracking-wider">Balance</span>
 <Button variant="ghost" size="sm" className="h-4 p-0 text-xs text-amber-400 hover:text-amber-300" onClick={() => setAdjModal(folio)}>
 Adjust
 </Button>
 </div>
 </div>
 ...
 <span className="text-xs font-bold text-blue-400 tracking-wider">Routing Instructions</span>
 ...
 <Badge variant="outline" className="text-xs py-0 border-border mr-2">{rule.criteria.category}</Badge>
 <MoveRight className="w-3 h-3 mr-2 text-muted-foreground" />
 ...
 <p className="text-xs text-muted-foreground">No routing applied.</p>
 )}
 </div>
 )}
 ...
 {item.isRouted && <span className="text-xs text-purple-400">Routed</span>}
 </td>
 ...
 <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-[#0a0a0f] border border-border rounded-lg shadow-sm p-6 z-50">
 <div className="flex justify-between items-center mb-6">
 <h2 className="text-xl font-bold text-white">Post Charge</h2>
 <button onClick={() => setChargeModal(false)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 <form onSubmit={(e) => {
 e.preventDefault()
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Amount ($)</label>
 <Input type="number" step="0.01" required value={chargeForm.amount} onChange={e => setChargeForm({ ...chargeForm, amount: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Category</label>
 <select value={chargeForm.category} onChange={e => setChargeForm({ ...chargeForm, category: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Description</label>
 <Input required value={chargeForm.description} onChange={e => setChargeForm({ ...chargeForm, description: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="e.g. Minibar consumption" />
 </div>
 ...
 <button onClick={() => setRoutingModal(null)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 <form onSubmit={(e) => {
 e.preventDefault()
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Charge Category to Route</label>
 <select value={routingForm.category} onChange={e => setRoutingForm({ ...routingForm, category: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 <option value="ROOM_CHARGE">Room Charge</option>
 <option value="MINIBAR">Minibar</option>
 <option value="SPA">Spa</option>
 <option value="RESTAURANT">Restaurant</option>
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Target Window Number</label>
 <Input type="number" min="2" required value={routingForm.targetWindow} onChange={e => setRoutingForm({ ...routingForm, targetWindow: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
 ...
 <button onClick={() => setAdjModal(null)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 <form onSubmit={(e) => {
 e.preventDefault()
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Amount to Deduct ($)</label>
 <Input type="number" step="0.01" required value={adjForm.amount} onChange={e => setAdjForm({ ...adjForm, amount: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Adjustment Type</label>
 <select value={adjForm.type} onChange={e => setAdjForm({ ...adjForm, type: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Managerial Reason</label>
 <Input required minLength={5} value={adjForm.reason} onChange={e => setAdjForm({ ...adjForm, reason: e.target.value })} className="bg-[#1a1a24] border-border text-white" placeholder="Reason for override..." />
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
