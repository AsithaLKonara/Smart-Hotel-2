...
export function BookingsClient({ initialBookings }: { initialBookings: Booking[] }) {
 ...
 useEffect(() => {
 // Only re-fetch if activePropertyId explicitly triggers it or session changes, but we skip initial waterfall
 if (status === 'authenticated' && session && activePropertyId) {
 fetchBookings()
 }
 }, [session, status, activePropertyId]) // eslint-disable-line react-hooks/exhaustive-deps
 ...
 switch (status) {
 case 'CONFIRMED': return 'bg-emerald-500/10 text-emerald-500 border-border'
 case 'CHECKED_IN': return 'bg-primary/10 text-primary border-primary/20'
 case 'CHECKED_OUT': return 'bg-white/10 text-muted-foreground border-border'
 case 'CANCELLED': return 'bg-rose-500/10 text-rose-500 border-border'
 default: return 'bg-amber-500/10 text-amber-500 border-border'
 }
 }
 ...
 <div>
 <p className="text-xs text-muted-foreground font-bold ">Total Active</p>
 ...
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs text-muted-foreground font-bold ">Confirmed</p>
 ...
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs text-muted-foreground font-bold ">In Residence</p>
 ...
 className="pl-12 pr-4 py-3 bg-card border border-border rounded-lg w-full text-white placeholder:text-muted-foreground focus:border-primary/50 transition-all outline-none"
 />
 </div>
 ...
 className="px-4 py-3 bg-card border border-border rounded-lg text-white outline-none focus:border-primary/50 transition-all text-xs font-bold "
 >
 ...
 <Button 
 variant="outline"
 className="px-6 py-3 bg-primary/10 border border-primary/20 hover:bg-primary/20 text-primary text-xs font-bold rounded-lg h-auto"
 onClick={() => {
 ...
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Booking</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Guest</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Room</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Dates</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Revenue</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Status</th>
 <th className="px-8 py-4 text-left text-xs font-bold text-muted-foreground ">Actions</th>
 </tr>
 </thead>
 ...
 <div className="text-xs text-muted-foreground font-bold">{booking.roomAssignments?.[0]?.room?.type || 'TBD'}</div>
 </td>
 <td className="px-8 py-6">
 ...
 <div className={cn("text-xs font-bold ", getPaymentStatusColor(booking.paymentStatus))}>
 {booking.paymentStatus}
 </div>
 </td>
 <td className="px-8 py-6">
 <Badge variant="outline" className={cn("text-xs font-bold ", getStatusColor(booking.status))}>
 ...
 <Button size="sm" className="bg-primary hover:bg-primary/90 text-white rounded-xl text-xs font-bold h-8 px-4" onClick={() => handleStatusUpdate(booking.id, 'CONFIRMED')}>
 Confirm
 </Button>
 )}
 {booking.status === 'CONFIRMED' && (
 <Button size="sm" className="bg-primary hover:bg-primary/90 text-white rounded-xl text-xs font-bold h-8 px-4" onClick={() => handleStatusUpdate(booking.id, 'CHECKED_IN')}>
 Check In
 </Button>
 )}
 {booking.status === 'CHECKED_IN' && (
 <Button size="sm" className="bg-primary hover:bg-primary/90 text-white rounded-xl text-xs font-bold h-8 px-4" onClick={() => handleStatusUpdate(booking.id, 'CHECKED_OUT')}>
 Check Out
 </Button>
 )}
 </div>
