...
export default function ForensicAuditTimeline() {
 ...
 if (action.includes('CHAOS')) return 'bg-amber-500/10 text-amber-400 border-border'
 if (action.includes('ROLLBACK')) return 'bg-purple-500/10 text-purple-400 border-purple-500/20'
 if (action.includes('BOOKING')) return 'bg-emerald-500/10 text-emerald-400 border-border'
 ...
 <Badge className="bg-rose-500/10 text-rose-400 border border-border text-xs tracking-wider font-bold py-1 px-3">
 ...
 <p className="text-muted-foreground text-sm mt-1">Immutable transaction history, live state inspection, operator validation & secure cryptographic rollbacks.</p>
 </div>
 <div className="flex items-center gap-3">
 <Button onClick={loadAuditLogs} variant="outline" size="sm" className="bg-muted/50 border-purple-900/50 text-purple-300 hover:bg-purple-900/30">
 ...
 <Card className="bg-muted/50 border border-purple-900/30 rounded-none shadow-xl">
 <CardHeader className="border-b border-border">
 <CardTitle className="text-sm font-bold text-slate-300">Auditing Scope</CardTitle>
 <CardDescription className="text-muted-foreground text-xs">Filter trail metrics sequentially</CardDescription>
 </CardHeader>
 ...
 <p className="text-xs font-bold text-muted-foreground mb-2">Category Filter</p>
 {uniqueActions.map(act => (
 <button
 key={act}
 onClick={() => setActionFilter(act)}
 className={`w-full text-left text-xs px-3 py-2 flex items-center justify-between border transition-all ${actionFilter === act ? 'bg-purple-950/40 border-purple-500/50 text-white font-bold' : 'bg-transparent border-transparent text-muted-foreground hover:text-slate-200 hover:bg-muted/30'}`}
 >
 <span>{act.replace('_', ' ')}</span>
 <Badge className="bg-muted/50 border-0 text-xs text-muted-foreground">
 ...
 <Card className="bg-muted/50 border border-purple-900/30 rounded-none shadow-xl min-h-[500px]">
 ...
 <CardDescription className="text-muted-foreground text-xs">A linear cryptographic timeline displaying database mutations.</CardDescription>
 </div>
 ...
 <AlertCircle className="w-10 h-10 text-muted-foreground animate-bounce" />
 ...
 {/* Timeline node node */}
 <span className="absolute -left-[41px] top-1.5 flex items-center justify-center w-6 h-6 rounded-full bg-background border border-purple-500/50 group-hover:border-purple-400 transition-colors">
 <Clock className="w-3.5 h-3.5 text-purple-400" />
 </span>

 <div className="p-4 bg-muted/30 border border-slate-800/80 hover:border-purple-900/30 transition-all flex flex-col gap-3">
 
 {/* Header section */}
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-900/40 pb-2.5">
 <div className="flex items-center gap-2.5">
 <Badge className={`text-xs tracking-wider font-bold px-2 py-0.5 border ${getActionBadgeColor(log.action)}`}>
 ...
 System registered action state <strong className="text-white">[{log.action}]</strong> executed by host address <code className="text-muted-foreground bg-slate-950 px-1 py-0.5 font-mono">{log.ip || '127.0.0.1'}</code>.
 </p>
 ...
 size="sm"
 className="bg-purple-900/40 hover:bg-purple-600 border border-purple-500/30 text-purple-200 hover:text-white h-7 text-xs rounded-none px-3 font-bold tracking-wider"
 >
 {isRollingBack ? (
 <>
