...
export default function LoyaltyPage() {
 ...
 <p className="text-muted-foreground">Manage VIP guests, tiers, and points</p>
 </div>
 ...
 <select required value={formData.accountId} onChange={e=>setFormData({...formData, accountId: e.target.value})} className="w-full p-2 rounded bg-black/50 border border-border mt-1">
 ...
 <div>
 <label className="text-xs text-muted-foreground">Points (+ to add, - to deduct)</label>
 <input type="number" required value={formData.points} onChange={e=>setFormData({...formData, points: parseInt(e.target.value)})} className="w-full p-2 rounded bg-black/50 border border-border mt-1" />
 </div>
 <div>
 <label className="text-xs text-muted-foreground">Reason / Description</label>
 <input required value={formData.description} onChange={e=>setFormData({...formData, description: e.target.value})} className="w-full p-2 rounded bg-black/50 border border-border mt-1" placeholder="e.g. Service recovery" />
 </div>
 </div>
 ...
 <th className="p-4 font-medium text-muted-foreground">Current Tier</th>
 <th className="p-4 font-medium text-muted-foreground text-right">Available Points</th>
 <th className="p-4 font-medium text-muted-foreground text-right">Lifetime Earned</th>
 </tr>
 </thead>
 ...
 <td className="p-4">
 <span className={`px-2 py-1 rounded border text-xs font-bold ${getTierColor(acc.tier)}`}>
 {acc.tier}
 </span>
 </td>
 ...
 <div className="p-12 text-center text-muted-foreground">No loyalty accounts found. Ensure users have been migrated or assigned points.</div>
 )}
 </div>
 </div>
 )
