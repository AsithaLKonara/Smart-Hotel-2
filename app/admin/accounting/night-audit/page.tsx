...
export default function NightAuditPage() {
 ...
 <Card className="bg-[#1a1a1a] border-border md:col-span-2 shadow-sm">
 ...
 onClick={runAudit} 
 disabled={running}
 className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm px-12 py-6 text-lg"
 >
 ...
 className="border-border text-amber-400 bg-amber-500/10 hover:bg-amber-500/20"
 >
 ...
 {healthStatus && (
 <div className="mt-4 w-full text-left bg-black/40 border border-border rounded-xl p-4 text-xs">
 <div className="flex items-center gap-2 mb-2">
 <span className={`w-2 h-2 rounded-full ${healthStatus.status === 'healthy' ? 'bg-emerald-400' : 'bg-rose-400'}`} />
 <span className={`font-bold ${healthStatus.status === 'healthy' ? 'text-emerald-400' : 'text-rose-400'}`}>
 {healthStatus.status}
 </span>
 <span className="text-muted-foreground ml-auto">{healthStatus.timestamp}</span>
 </div>
 ...
 <Card className="bg-[#1a1a1a] border-border border-l-4 border-l-yellow-500">
 ...
 <Card className="bg-[#1a1a1a] border-border border-l-4 border-l-primary">
 <CardContent className="p-4 flex items-start gap-4">
 ...
 {logs.length === 0 && (
 <tr>
 <td colSpan={5} className="p-8 text-center text-muted-foreground">No night audit logs found.</td>
 </tr>
 )}
 </tbody>
 </table>
