...
export default function PaymentsLedgerPage() {
 ...
 <td className="p-4 text-white/70">{p.paymentMethod}</td>
 <td className="p-4">
 ...
 className="px-4 py-2 bg-amber-500/20 text-amber-400 border border-border rounded-lg hover:bg-amber-500/30 transition-all text-sm font-semibold">
 Fiscal Sign Invoice
 </button>
 </div>
 </div>
 ...
 <button onClick={() => setConvertModal(false)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 {convertResult ? (
 <div className="p-4 bg-emerald-500/10 border border-border rounded-xl mb-4 text-emerald-400 text-center text-sm font-bold">{convertResult}</div>
 ) : null}
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Folio ID</label>
 <Input required value={convertForm.folioId} onChange={e => setConvertForm({ ...convertForm, folioId: e.target.value })} className="bg-[#1a1a24] border-border text-white font-mono text-sm" placeholder="UUID of the folio" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Target Currency</label>
 <select value={convertForm.currency} onChange={e => setConvertForm({ ...convertForm, currency: e.target.value })} className="w-full bg-[#1a1a24] border border-border rounded-lg p-2.5 text-sm text-white">
 ...
 <button onClick={() => setSplitModal(false)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 <form onSubmit={async (e) => {
 e.preventDefault(); setSubmitting(true)
 try {
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Original Folio ID</label>
 <Input required value={splitForm.folioId} onChange={e => setSplitForm({ ...splitForm, folioId: e.target.value })} className="bg-[#1a1a24] border-border text-white font-mono text-sm" placeholder="UUID of the original folio" />
 </div>
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Split Percentage (%)</label>
 <Input type="number" min="1" max="99" required value={splitForm.percentage} onChange={e => setSplitForm({ ...splitForm, percentage: e.target.value })} className="bg-[#1a1a24] border-border text-white" />
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
 ...
 <button onClick={() => setFiscalModal(false)} className="text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
 </div>
 {fiscalResult ? (
 <div className="p-4 bg-amber-500/10 border border-border rounded-xl mb-4 text-amber-400 text-xs font-mono break-all">{fiscalResult}</div>
 ) : null}
 <form onSubmit={async (e) => {
 e.preventDefault(); setSubmitting(true)
 try {
 ...
 <div>
 <label className="text-xs text-white/60 font-bold tracking-wider mb-2 block">Invoice ID</label>
 <Input required value={fiscalForm.invoiceId} onChange={e => setFiscalForm({ invoiceId: e.target.value })} className="bg-[#1a1a24] border-border text-white font-mono text-sm" placeholder="Invoice UUID" />
 </div>
 <div className="flex justify-end gap-3 pt-4 border-t border-border">
