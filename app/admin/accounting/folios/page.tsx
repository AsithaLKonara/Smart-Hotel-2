...
export default function FoliosManagementPage() {
 ...
 <thead className="bg-muted/50 text-white/50 text-xs ">
 <tr>
 ...
 return (
 <tr key={folio.id} className="hover:bg-muted/50">
 ...
 <td className="p-4 text-white/70">{folio.type}</td>
 <td className="p-4">${total.toFixed(2)}</td>
 <td className="p-4 font-semibold text-primary">${balance.toFixed(2)}</td>
 <td className="p-4">
 <span className={`px-2 py-1 rounded text-xs font-bold tracking-wider ${folio.status === 'PAID' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
 {folio.status}
 </span>
 </td>
 ...
 <DropdownMenu.Portal>
 <DropdownMenu.Content className="min-w-[180px] bg-[#1a1a1a] border border-border rounded-xl p-1 shadow-xl text-sm text-white/90 z-50">
 ...
 {folios.length === 0 && (
 <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">No folios found.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 {/* Drawer Container */}
 <div className="relative w-full max-w-lg bg-[#111116] border-l border-border shadow-sm flex flex-col h-full z-50 text-white animate-in slide-in-from-right duration-300 overflow-hidden">
 {/* Header */}
 <div className="p-6 border-b border-border flex justify-between items-start bg-black/30">
 <div>
 <div className="flex items-center gap-2 mb-1">
 <h2 className="text-xl font-bold text-white">Folio Statement</h2>
 <span className={`px-2 py-0.5 rounded text-xs font-bold tracking-wider ${selectedFolioForDrawer.status === 'PAID' ? 'bg-emerald-500/20 text-emerald-400 border border-border' : 'bg-amber-500/20 text-amber-400 border border-border'}`}>
 {selectedFolioForDrawer.status}
 </span>
 </div>
 <p className="text-xs font-mono text-muted-foreground">ID: {selectedFolioForDrawer.id}</p>
 ...
 <div className="p-6 bg-muted/50 border-b border-border grid grid-cols-3 gap-3 text-center">
 <div className="bg-black/30 p-3 rounded-xl border border-border">
 ...
 <div className="bg-black/30 p-3 rounded-xl border border-border">
 <p className="text-xs font-bold text-muted-foreground">Total Payments</p>
 <p className="text-sm font-bold text-emerald-400 mt-1">
 ${(selectedFolioForDrawer.payments || []).reduce((a: number, b: any) => a + Number(b.amount || 0), 0).toFixed(2)}
 </p>
 </div>
 <div className="bg-black/30 p-3 rounded-xl border border-border">
 <p className="text-xs font-bold text-muted-foreground">Current Balance</p>
 ...
 {/* Line Items List */}
 <div>
 <h3 className="text-xs font-bold tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
 ...
 <div key={item.id} className="bg-muted/50 border border-border rounded-xl p-4 flex justify-between items-center hover:border-white/20 transition-all">
 <div className="space-y-1">
 <p className="font-semibold text-sm text-white">{item.description}</p>
 <div className="flex items-center gap-2">
 <span className="text-xs bg-white/10 px-2 py-0.5 rounded text-slate-300 font-bold">
 ...
 <div key={p.id} className="bg-emerald-500/10 border border-border rounded-xl p-4 flex justify-between items-center">
 <div>
 ...
 <div className="p-4 border-t border-border bg-black/40 grid grid-cols-3 gap-2">
 <Button
 variant="outline"
 onClick={() => {
 ...
 className="bg-amber-500/10 border-border text-amber-300 hover:bg-amber-500/20 text-xs font-bold"
 >
 ...
 <DialogContent className="bg-[#121212] border-border text-white max-w-md rounded-lg p-6">
 ...
 className="bg-muted/50 border-border text-white font-mono text-sm"
 required
 />
 </div>

 <DialogFooter className="pt-4 gap-2 border-t border-border">
 <Button type="button" variant="ghost" onClick={() => setActiveModal(null)} className="hover:bg-white/10 text-muted-foreground hover:text-white">
 Cancel
 </Button>
 ...
 className="w-full bg-muted/50 border border-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
 >
 ...
 <div className="p-4 bg-amber-500/10 border border-border rounded-xl space-y-2">
 <p className="text-xs text-amber-400 font-bold tracking-wider">Cryptographic Signature</p>
 <p className="font-mono text-xs text-slate-200 break-all bg-black/40 p-2 rounded border border-border">{fiscalResult.signature}</p>
 <p className="text-xs text-muted-foreground">Timestamp: {new Date(fiscalResult.timestamp).toLocaleString()}</p>
 </div>
 ...
 <div className="p-4 bg-muted/50 border border-border rounded-xl text-xs text-slate-300 space-y-1">
