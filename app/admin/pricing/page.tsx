...
export default function PricingOperations() {
 ...
 <p className="text-muted-foreground text-sm mt-1">Manage dynamic rate plans, seasonal overrides, and quoting.</p>
 </div>
 <div className="flex gap-2 mt-4 md:mt-0">
 <Button variant="outline" className="bg-muted/50 border-border text-white" onClick={() => setActiveTab('RATE_PLANS')}>
 <Settings2 className="w-4 h-4 mr-2" /> Rate Plans
 </Button>
 <Button variant="outline" className="bg-muted/50 border-border text-white" onClick={() => setActiveTab('CALENDAR')}>
 ...
 <Card key={rp.id} className="bg-muted/50 border-border shadow-lg">
 ...
 <p className="text-sm text-muted-foreground mt-1">Room Type: {rp.roomType?.name}</p>
 </div>
 <div className="text-right">
 <span className="text-sm text-muted-foreground block mb-1">Base Multiplier</span>
 ...
 <h4 className="text-sm font-bold tracking-wider text-muted-foreground">Seasonal Overrides</h4>
 ...
 <p className="text-xs text-muted-foreground">No seasonal rules applied.</p>
 ) : (
 <div className="space-y-2">
 {rp.seasonalRates.map((season: any) => (
 <div key={season.id} className="flex justify-between items-center p-3 bg-black/40 rounded-lg border border-border">
 <div>
 <span className="font-bold text-sm text-white block">{season.name}</span>
 <span className="text-xs text-muted-foreground">{format(new Date(season.startDate), 'MMM d')} - {format(new Date(season.endDate), 'MMM d')}</span>
 </div>
 <Badge className="bg-amber-500/10 text-amber-500 border-border font-mono">
 ...
 <Card className="bg-[#0a0a0f] border-border">
 <CardContent className="p-6 space-y-4">
 {yieldRules?.length === 0 ? (
 <p className="text-xs text-muted-foreground">No active yield rules configured.</p>
 ) : (
 yieldRules?.map((rule: any) => (
 <div key={rule.id} className="p-4 bg-emerald-500/10 border border-border rounded-xl">
 ...
 <div className="p-4 bg-muted/50 border border-border rounded-xl opacity-50">
 <div className="flex justify-between items-center mb-2">
 <span className="font-bold text-white">Baseline Weekend Premium</span>
 <Badge className="bg-slate-500/20 text-muted-foreground">Default</Badge>
 </div>
 <p className="text-xs text-muted-foreground">Automatically increases base fallback rates by 15% on weekends.</p>
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 <div 
 key={idx} 
 className={`p-4 rounded-xl border ${applicableRule ? 'bg-amber-500/5 border-border' : isWeekend ? 'bg-primary/5 border-primary/20' : 'bg-muted/50 border-border'} min-h-[100px] flex flex-col justify-between`}
 >
 ...
 <span className={`text-xl font-bold ${applicableRule ? 'text-amber-500' : isWeekend ? 'text-primary' : 'text-white'}`}>
 ${Math.round(baseRate)}
 </span>
 </div>
 </div>
