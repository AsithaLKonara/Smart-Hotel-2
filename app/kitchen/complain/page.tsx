...
export default function KitchenComplaintsPage() {
 ...
 <p className="text-xs text-muted-foreground">Direct line to Executive Operations</p>
 </div>
 </div>

 <form onSubmit={handleSubmit} className="space-y-6">
 <div className="space-y-2">
 <label className="text-xs font-bold text-muted-foreground ml-2">Escalation Subject</label>
 <input 
 type="text" 
 value={formData.subject}
 onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
 placeholder="e.g. Supply Shortage - Fresh Seafood"
 className="w-full bg-muted/50 border border-border rounded-lg p-4 text-sm text-white focus:ring-2 focus:ring-red-500/40 outline-none"
 disabled={submitting}
 />
 </div>

 <div className="space-y-2">
 <label className="text-xs font-bold text-muted-foreground ml-2">Category</label>
 <select 
 value={formData.category}
 onChange={(e) => setFormData({ ...formData, category: e.target.value })}
 className="w-full bg-muted/50 border border-border rounded-lg p-4 text-sm text-white focus:ring-2 focus:ring-red-500/40 outline-none appearance-none"
 disabled={submitting}
 >
 ...
 <div className="space-y-2">
 <label className="text-xs font-bold text-muted-foreground ml-2">Situation Details</label>
 <textarea 
 value={formData.description}
 onChange={(e) => setFormData({ ...formData, description: e.target.value })}
 placeholder="Provide context for the Duty Manager..."
 className="w-full bg-muted/50 border border-border rounded-lg p-4 text-sm text-white focus:ring-2 focus:ring-red-500/40 outline-none h-40 resize-none"
 disabled={submitting}
 />
 </div>
 ...
 <Card className="bg-primary/5 border-primary/20 p-8 rounded-xl space-y-4">
 ...
 <Badge className="bg-emerald-500/10 text-emerald-500 text-xs">Online</Badge>
 </div>
 <div className="flex items-center justify-between p-3 bg-black/20 rounded-xl">
 ...
 <h4 className="text-xs font-bold text-muted-foreground px-2 flex items-center gap-2">
 ...
 <p className="text-xs text-muted-foreground font-bold">2h ago • Resolved</p>
 </div>
 </div>
 ...
 <p className="text-xs text-muted-foreground font-bold">5h ago • Awaiting Source</p>
 </div>
 </div>
 </div>
 </Card>
