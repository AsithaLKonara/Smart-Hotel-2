...
function KitchenDashboardContent() {
 ...
 className="bg-rose-500/10 text-rose-400 border border-border hover:bg-rose-500 hover:text-white h-10 px-4 text-xs font-bold rounded-xl"
 >
 ...
 <span className="font-bold text-white text-xs">New Orders</span>
 <Badge className="bg-amber-500/20 text-amber-500 border-border">{ordersByStatus.PENDING.length}</Badge>
 </div>
 <div className="p-6 space-y-4">
 {ordersByStatus.PENDING.map(ord => (
 <div key={ord.id} className="p-4 bg-muted/50 border border-border rounded-lg space-y-4 hover:border-primary/30 transition-all group">
 <div className="flex justify-between font-bold text-sm">
 <span className="text-muted-foreground">#{ord.orderNumber}</span>
 <span className="text-primary font-bold">Room {ord.roomNumber}</span>
 </div>
 ...
 className="w-full h-10 text-xs font-bold bg-primary hover:bg-primary/90 text-white rounded-xl"
 ...
 {/* Confirmed */}
 <Card className="bg-card border-border rounded-xl shadow-sm overflow-hidden">
 <div className="p-6 border-b border-border flex justify-between items-center bg-blue-500/5">
 <span className="font-bold text-white text-xs">Confirmed</span>
 ...
 <div key={ord.id} className="p-4 bg-muted/50 border border-border rounded-lg space-y-4">
 <div className="font-bold text-sm text-white">#{ord.orderNumber} - Room {ord.roomNumber}</div>
 <Button 
 onClick={() => updateOrderStatus(ord.id, 'PREPARING')}
 className="w-full h-10 text-xs font-bold bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl"
 disabled={updatingOrder === ord.id}
 >
 Start Preparation
 </Button>
 ...
 <span className="font-bold text-white text-xs">Preparing</span>
 ...
 <div key={ord.id} className="p-4 bg-muted/50 border border-border rounded-lg space-y-4 border-l-4 border-l-purple-500">
 <div className="font-bold text-sm text-white flex items-center justify-between">
 <div className="flex items-center gap-2">
 <ChefHat className="w-4 h-4 text-purple-500" /> #{ord.orderNumber}
 </div>
 <span className="text-purple-500 font-bold">Room {ord.roomNumber}</span>
 </div>
 <Button 
 onClick={() => updateOrderStatus(ord.id, 'READY')}
 className="w-full h-10 text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white rounded-xl animate-pulse"
 disabled={updatingOrder === ord.id}
 >
 Mark as Ready
 </Button>
 ...
 <span className="font-bold text-white text-xs">Ready</span>
 <Badge className="bg-emerald-500/20 text-emerald-500 border-border">{ordersByStatus.READY.length}</Badge>
 </div>
 <div className="p-6 space-y-4">
 {ordersByStatus.READY.map(ord => (
 <div key={ord.id} className="p-4 bg-emerald-500/10 border border-border rounded-lg space-y-4">
 ...
 className="w-full h-10 text-xs font-bold bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-900/20"
 disabled={updatingOrder === ord.id}
 >
 Finish & Deliver
 </Button>
