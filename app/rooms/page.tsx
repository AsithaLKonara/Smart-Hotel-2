...
export default function RoomsPage() {
 ...
 {/* Contextual Hero */}
 <section className="relative min-h-[500px] flex items-center justify-center overflow-hidden border-b border-border">
 <Image 
 ...
 {/* Filter Bar */}
 <section className="sticky top-20 z-40 bg-black/50 backdrop-blur-xl border-b border-border py-6">
 ...
 className="w-full pl-12 pr-4 py-3 bg-muted/50 border border-border text-white placeholder:text-muted-foreground focus:ring-1 focus:ring-primary transition-all text-sm rounded-xl"
 value={searchTerm}
 ...
 className={`px-6 py-2 text-xs font-bold whitespace-nowrap transition-all border rounded-lg ${
 selectedType === type ? 'bg-primary/20 text-primary border-primary/30' : 'border-border text-muted-foreground hover:border-primary/30 hover:text-primary'
 }`}
 >
 {type}
 </button>
 ...
 <h3 className="text-2xl font-serif text-muted-foreground">No suites match your search.</h3>
 </div>
 ) : (
 <motion.div 
 ...
 <div className="bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-lg border border-border">
 <span className="text-xs text-white font-bold">{room.type}</span>
 </div>
 </div>
 ...
 <div className="flex items-center gap-1"><Users className="w-3 h-3" /><span className="text-xs tracking-wider">{room.capacity} Guests</span></div>
 <div className="flex items-center gap-1"><Wind className="w-3 h-3" /><span className="text-xs tracking-wider">{room.size || '45'} m²</span></div>
 </div>
 ...
 <Button variant="outline" className="w-full border-white/30 text-white rounded-xl h-10 text-xs font-bold hover:bg-white/20 transition-all backdrop-blur-sm">
 Details
 </Button>
 </Link>
 <Link href={`/booking?roomType=${room.type}`} className="flex-1">
 <Button className="w-full bg-primary/10 text-white rounded-xl h-10 text-xs font-bold border-none shadow-sm">
 Reserve
 </Button>
 </Link>
 </div>
 ...
 <span className="text-primary font-serif italic text-sm">{formatPrice(room.price)}<span className="text-xs text-muted-foreground not-italic ml-1">/night</span></span>
 </div>
 <p className="text-xs text-muted-foreground font-light leading-relaxed line-clamp-1">
 ...
 {/* CTA Section */}
 <section className="py-24 bg-black/30 backdrop-blur-md border-t border-border">
 ...
 <Link href="/contact">
 <Button variant="outline" className="border-primary text-primary rounded-xl px-12 h-14 text-xs font-bold hover:bg-primary hover:text-white transition-all">
 Contact Concierge
 </Button>
 </Link>
 </div>
