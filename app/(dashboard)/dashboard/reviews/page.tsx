...
export default function ReviewsPage() {
 ...
 <Card className="bg-card border-border p-10 rounded-xl space-y-10 relative overflow-hidden">
 <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 blur-[100px] -mr-32 -mt-32" />
 
 <div className="space-y-2 relative z-10">
 <h4 className="text-2xl font-serif font-bold text-white">Share Your Thoughts</h4>
 <p className="text-sm text-muted-foreground">How would you rate your current stay?</p>
 </div>
 ...
 <label className="text-xs font-bold text-muted-foreground ml-2">Your Experience</label>
 <textarea 
 value={comment}
 onChange={(e) => setComment(e.target.value)}
 placeholder="What made your stay special? (Optional)"
 className="w-full bg-muted/50 border border-border rounded-xl p-6 text-sm text-white placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/40 outline-none h-48 resize-none"
 disabled={submitting}
 />
 </div>
 ...
 disabled={submitting}
 className="w-full h-16 bg-primary/10 text-white rounded-lg font-bold tracking-[0.2em] text-xs shadow-sm transition-all hover:scale-[1.02]"
 >
 ...
 <h4 className="text-xs font-bold text-muted-foreground px-2">Your History</h4>
 {loading ? (
 <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div>
 ) : reviews.map((rev) => (
 <Card key={rev.id} className="bg-card border-border p-8 rounded-[30px] space-y-4">
 ...
 <p className="text-xs text-muted-foreground font-bold ">No review history</p>
 </div>
 )}
 </div>
 </div>
