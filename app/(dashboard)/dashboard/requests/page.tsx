...
export default function RequestsPage() {
 ...
 <Card 
 key={cat.id} 
 className="bg-card border-border p-6 rounded-[30px] hover:border-primary/30 transition-all group cursor-pointer"
 ...
 className="w-full bg-muted/50 border border-border rounded-lg p-4 text-sm text-white placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/40 outline-none h-32 resize-none"
 />
 <Button 
 disabled={!quickRequest.trim() || submitting}
 onClick={() => submitRequest('GUEST_REQUEST', 'Quick Request', quickRequest)}
 className="w-full h-14 bg-primary hover:bg-primary/90 text-white rounded-lg font-bold text-xs"
 >
 ...
 <h4 className="text-sm font-bold text-muted-foreground">Recent Activity</h4>
 <Badge variant="outline" className="text-xs border-border text-primary">Live Updates</Badge>
 </div>
 ...
 <Card key={req.id} className="bg-card border-border p-6 rounded-xl group">
 ...
 <span className="text-xs font-bold text-muted-foreground">{req.id.slice(-8)}</span>
 </div>
 ...
 {req.description && <p className="text-xs text-muted-foreground mb-3 line-clamp-2">{req.description}</p>}
 <div className="flex items-center justify-between text-xs font-bold">
 <span className="text-muted-foreground">{req.type}</span>
 <span className="text-muted-foreground">{new Date(req.createdAt).toLocaleTimeString()}</span>
 </div>
 </Card>
 )) : (
 <div className="py-20 text-center text-muted-foreground font-bold text-xs">
